...
- **Must:** toda decisión de back-pressure por memoria debe basarse en el **RSS real del proceso** — `crate::metrics::memory_breakdown_snapshot().process_rss_bytes` (medido vía Win32 `QueryWorkingSetEx` / Mach `task_info` / sysinfo en `src/metrics/core/mod.rs:471`) — o en una estimación lógica explícitamente documentada como tal.
...
- **Por qué:** `check_memory_pressure` (`src/storage/engine/stats.rs:98`) hoy usa `effective_bytes()` = `physical_rss.unwrap_or(logical)`; con `mmap_hnsw=true` (default) el `physical_rss` solo cuenta páginas mmap residentes y **oculta** el heap (structs HNSW, neighbor lists, WAL buffers, backend cache, text index). El RSS real ya se mide en `flush()` (`record_memory_breakdown`) pero no alimenta el check → el guard no puede prevenir OOM.
