...
.EXAMPLE
    # Diagnóstico rápido
    .opencode\task-system\self-modification\dgm-loop.ps1 -Quick

    # Loop completo (genera propuesta)
    .opencode\task-system\self-modification\dgm-loop.ps1

    # Probar propuesta sin riesgo
    .opencode\task-system\self-modification\dgm-rehearse.ps1 -ProposalFile proposals/dgm-proposal-NNN.json -Quick

    # Promover (solo si rehearsal pasó)
    .opencode\task-system\self-modification\dgm-promote.ps1 -ProposalFile proposals/dgm-proposal-NNN.json -RehearsalId <ID>
#>
...
$ReportDir = Join-Path $PipelinePath ".opencode" "task-system" "traces"
$ProposalsDir = Join-Path $PipelinePath ".opencode" "task-system" "self-modification" "proposals"
$DiagnosisScript = Join-Path $PipelinePath ".opencode" "task-system" "self-modification" "performance_diagnosis.py"
...
Write-Host "    .opencode\task-system\self-modification\dgm-rehearse.ps1 -ProposalFile $proposalFile -Quick" -ForegroundColor Yellow
Write-Host ""
Write-Host "  ▶ Si el rehearsal pasa y querés promover:" -ForegroundColor Cyan
Write-Host "    .opencode\task-system\self-modification\dgm-promote.ps1 -ProposalFile $proposalFile -RehearsalId <ID>" -ForegroundColor Yellow
Write-Host ""
