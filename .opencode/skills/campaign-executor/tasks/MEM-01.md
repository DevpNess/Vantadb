...
- [ ] **S7 — Cierre**: verify full: `cargo fmt --check`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo nextest run --profile audit --workspace --build-jobs 2`, `cargo doc -p vantadb --no-deps`. Commit convencional (sin push). Actualizar plan file MEM-01 → completed.
