...
class MemoryClient:
    """Grouped view over ``VantaDB`` memory-record methods (same signatures)."""
...
class GraphClient:
    """Grouped view over ``VantaDB`` graph methods (same signatures).

    ``insert``/``get``/``delete`` are node-level ops (``id: u128``) — see
...
class SystemClient:
    """Catch-all grouped view over ``VantaDB`` system methods."""
    ...
    def compact_layout(self) -> int: ...
    def rebuild_index(self) -> dict: ...
    def reindex_hnsw_from_text(
        self, namespace: str, page_size: int
    ) -> dict: ...
    def repair_text_index(self) -> dict: ...
    def audit_text_index(
        self, namespace: str | None = None, deep: bool = False
...
class WikiClient:
    """Grouped view over ``VantaDB`` wiki summary-archive recovery."""
