...
| ID | Descripción (→ Resultado) | Archivos | Esfuerzo | Prio | Estado |
|----|-------------|----------|----------|------|--------|
| `MEM-01` | **F1 Search profile por namespace en core** — nuevo tipo **`SearchProfileConfig`** (D14; NO colisiona con el `SearchProfile` profiler I/O de `src/index/search/profile.rs`) con campos `mode: keyword\|vector\|hybrid`, `rrf_k`, `candidate_k` en `VantaMemorySearchRequest` con serde defaults retrocompatibles; parametriza `fuse_rrf`/`hybrid_candidate_budget` de `planner.rs` (hoy constantes globales RRF_K=60, CANDIDATE_MULTIPLIER=4, clamp 32/256). **Exposición en IQL + API + MCP (D13):** sintaxis IQL opcional compatible con la emitida por `server_client.rs` (`FROM <kind> WHERE <field> ~ "<text>" min = <threshold>` + cláusula profile) para que Studio lo use sin cambios de bridge; report RRF debe incluir `rrf_k` usado (el frontend Studio hardcodea RRF_K=60 — `desktop/scripts/selfcheck-retrieval.ts`). → búsqueda configurable por namespace. | `src/planner.rs`, `src/sdk/serialization/vector_types.rs`, `src/sdk/types.rs`, `src/sdk/search/mod.rs`, `src/cli_server.rs` (parser IQL) | 🟡 | 🔴 | ✅ Completado (plan 2026-08-18-vanta-memory, 2026-08-20) |
| `MEM-02` | **F1 Exponer search profile en MCP/search** — parámetro opcional `search_profile` (mode/rrf_k/candidate_k) en el tool MCP de búsqueda → passthrough a `VantaMemorySearchRequest`; sin profile → defaults de MEM-01. **Además de IQL (D13):** el tool MCP y el request nativo comparten el mismo `SearchProfileConfig`; verificar que IQL/API/MCP producen idénticos resultados (tests de paridad). → agentes controlan recall/precisión. | `vantadb-mcp/src/handlers/tools.rs`, `vantadb-mcp/src/validation.rs`, `vantadb-mcp/src/config.rs`, `src/cli_server.rs` | 🟢 | 🔴 | ✅ Completado (plan 2026-08-18-vanta-memory, 2026-08-20) |
| `MEM-03` | **F2 Entidades `entity_*` + CRUD en core** — modelo teams/users/agents/tasks/assets (hoy NO existe; modelo actual namespace+key): `UserEntity{user_type}`, `TeamEntity{owner_user_id, TeamRole}`, `AssetEntity{AssetType, AssetVisibility=private/team/restricted/agent/task, AssetStatus}`, `InjectionMode`; CRUD mínimo; protección `last_system_admin` (409). Decisión pendiente: tablas propias vs nodos partición InternalMetadata (patrón thread.rs). | `src/entity.rs` (nuevo), `src/entity/tests.rs`, `src/lib.rs` | 🟡 | 🔴 | ✅ Completado (plan 2026-08-18-vanta-memory, 2026-08-20) |
| `MEM-04` | **F2 Permission-checker allow-only** — cadena `resource→owner→member→visibility→role-default→ACL→deny` (96 líneas reales, allow-only: `effect=deny` NO deniega); `visibility=restricted` evalúa ACL inline antes de role-default; sysadmin no bypassa; recurso inexistente → `asset_not_available` anti-enumeración. ⚠️ Contradicción SYNTHESIS (96 líneas) vs 06 §6 (~40 líneas) a resolver. `src/rbac.rs` es dead code — decidir reemplazo vs coexistencia. | `src/permissions.rs` (nuevo) o `src/entity/checker.rs`, `src/lib.rs`, `src/rbac.rs` | 🟡 | 🔴 | ✅ Completado (plan 2026-08-18-vanta-memory, 2026-08-20) |
| `MEM-05` | **F2 Auth 3 capas en server** — L1 Bearer con `timingSafeEqual`; L2 header service-id → 401 si falta; L3 user-key → userId/isSystemAdmin desde entity_*; extiende `AuthState`+`auth_middleware` existentes (`cli_server.rs:275-351`); `/health` pública. **Añade el audit log server `/api/v2/audit` (D15)** — JSONL protegido por la misma auth, base del contrato 3 con Studio. Quota/billing DIFERIDO (SYNTHESIS §3). | `src/cli_server.rs`, `vantadb-server/src/middleware.rs`, `vantadb-server/src/server.rs`, `vantadb-server/src/audit.rs` (crear) | 🟢 | 🔴 | ✅ Completado (plan 2026-08-18-vanta-memory, 2026-08-20) |
| `MEM-06` | **F3 Esquema skills multi-versión en core** — VantaDB no es SQL: namespace `skills` + nodos por versión con metadata (`version`, `is_head`, `content_hash`, `expires_at`, owner_agent), índice único parcial por `(owner,name)` donde is_head, optimistic lock `expected_version`, TTL keep-recent=3, idempotencia por content-hash; CRUD create/update/patch/delete/versions. Reusar text_index + HNSW propios (NO vec0). | `src/skills.rs` (crear), `src/sdk/types.rs`, `src/lib.rs`, `tests/` | 🟡 | 🔴 | ✅ Completado (plan 2026-08-18-vanta-memory, 2026-08-20) |
| `MEM-07` | **F3 MCP tools skill_*** — 6 tools del review agent (`skill_list/view/create/update/patch/files_write`) sobre MEM-06; writes exigen `expected_version`; owner check → 404 sin filtrar existencia; límites 5MB/recurso, 50MB/skill. | `vantadb-mcp/src/handlers/tools.rs`, `vantadb-mcp/src/skills.rs` (crear) | 🟡 | 🔴 | ✅ Completado (plan 2026-08-18-vanta-memory, 2026-08-20) |
...
| `MEM-34` | **F1 Core Telemetría por capa (ADELANTADA de transversal a F1, D17)** — extender `operational_metrics_snapshot()` (`src/metrics/core/mod.rs:545`, hoy startup/wal/ann/text_index/hybrid) con latencias L1/L2/L3/recall/offload, `persona_length_before/after`, `persona_drift_ratio`, `recall_hit_count/top_score/strategy`; error-silent (AtomicU64, patrón existente — sin tests covering hoy, exigir tests); exponer por `/metrics`. **Studio ya consume `metrics()`/`namespaceStats()` (VS-CORE-02 en `desktop/src/vanta.ts`)** → el desktop gana las latencias por capa sin código nuevo. Contrato 3: **audit log en server `/api/v2/audit` (D15)** — JSONL compartido, Memory escribe eventos L1/L2/L3/offload; Studio lo lee en server mode. (Fusión de propuestas duplicadas de 2 agentes.) | `src/metrics/core/mod.rs`, `src/metrics/core/state.rs`, `src/cli_server.rs`, `vantadb-server/src/` (audit log), hooks en `vanta-memory/src/` | 🟡 | 🔴 | ✅ Completado (plan 2026-08-18-vanta-memory, 2026-08-20) |
| `MEM-35` | **Core Data plane de referencia en server** — rutas **REST** `POST /conversation/add` (01 §10, vía ThreadStore) y `GET /skill/listing` (03 §4, sobre MEM-06 o stub 501 si no existe) protegidas por auth existente; **orientado a AGENTES, no a Studio (D18)** — `/conversation/add` es pipeline multi-paso, forzarlo a IQL contaminaría el query language; Studio es viewer y no ingesta; si Studio lista skills algún día → 1 wrapper en `server_client.rs` (documentado, trivial). NO copiar `/v3/session/init` ni `/v3/knowledge/query` (no existen en TDAM — endpoints propios VantaDB). | `src/cli_server.rs`, `vantadb-server/src/server.rs`, `vantadb-server/tests/e2e.rs` | 🟢 | 🟡 | ✅ Completado (plan 2026-08-18-vanta-memory, 2026-08-20) |
