...
| `TECH-02` | **🐛 WASM: `reindexHnswFromText` lanza WASM_ERROR** — El método SÍ existe en `vantadb-wasm/src/lib.rs:788` pero el `pkg/` publicado está stale (por eso `vantadb-ts/src/vantadb.ts:547` lanza error). **Pipeline:** 🔍 Inv — comparar `pkg/vantadb_wasm.js`/.d.ts contra `lib.rs`; 📊 Análisis — confirmar que es issue de build/publish, no de código; ✅ Verif — regenerar `pkg/` y correr test browser (wasm-pack test --chrome); 🔧 Impl — rebuild `vantadb-wasm` y re-publicar `pkg/`. **DoD:** `reindexHnswFromText` funciona en TS/WASM; `pkg/` regenerado y versionado. Origen: hallazgo §6.8 de `docs/Investigaciones/DESKTOP-01b-investigacion-6-integraciones-arquitectura.md`. | `vantadb-wasm/pkg/`, `vantadb-wasm/src/lib.rs` | 🟢 2-4h | 🟡 |
...
| `INV-009-B` | **Phrase queries: cerrar gaps restantes** — (1) `Condition::TextMatch(field, query)` en parser IQL con reuso `string_literal`; (2) tokenización literal de frases (sin stopwords/stemming) en `query_plan_with_config`; (3) `highlight_phrases` + priorizar frase en window de snippet. El enforcement de frases en query execution YA existe (`src/sdk/search/mod.rs:416-425`). (INV-009 §6) | `src/parser/mod.rs`, `src/query.rs`, `src/sdk/search/snippet.rs` | 🟡 1-2d | 🟡 | ⏳ Pendiente |
...
| `INV-016-B` | **Motion tokens de duración/easing** — Definir CSS vars `--duration-fast/normal/slow` + `--ease-default` en `globals.css` `@theme inline`; reemplazar `cubic-bezier(0.2,0.8,0.2,1)` hardcodeado (~15 lugares: reveal, faq-section, page-transition, latency-comparator, benchmark-race, ~10 utilities/keyframes) + mapa JS para framer-motion/animejs (INV-016). | `web/src/app/globals.css`, `web/src/components/vanta/*.tsx`, `web/src/components/vanta/benchmark-race.tsx` | 🟢 1-2h | 🟢 | ⏳ Pendiente |
...
| `AUD-004` | **`query_lisp` tool MCP expone API que el core rechaza** — **Investigación:** `vantadb-mcp/src/lib.rs:864` registra tool `query_lisp` que llama a core sin gate; core exige feature `experimental-lisp` y no hay `src/parser/lisp.rs`; test `test_execute_hybrid_rejects_lisp` (`src/executor.rs:577`). **Análisis:** decidir gate por feature o eliminar la tool hasta implementarla. **Implementación:** gate por `#[cfg(feature="experimental-lisp")]` (evita tool rota en default) y documentar en MCP.md. | `vantadb-mcp/src/lib.rs`, `Cargo.toml` | 🟡 1-2h | 🔴 | ⏳ Pendiente |
| `AUD-005` | **Versionado inconsistente docs/api (3 fuentes)** — **Investigación:** openapi.yaml=0.4.0, MCP.md=0.1.5, HTTP_API.md=0.0.4 vs workspace=0.5.0. **Análisis:** elegir única fuente de versión (lineá a workspace definida en R-2 api-contract). **Implementación:** sincronizar cabeceras de los 3 docs + gate CI de versión. | `docs/api/openapi.yaml`, `docs/api/MCP.md`, `docs/api/HTTP_API.md` | 🟢 1-2h | 🟡 | ⏳ Pendiente |
| `AUD-006` | **MCP.md: 8 de 15 tools reales sin documentar** — **Investigación:** `src/lib.rs:808-964` define 15 tools MCP; MCP.md solo documenta 7. Faltan: `query_lisp`, `get_node_neighbors`, `inject_context`, `read_axioms`, `collection_stats`, `collection_list`, `collection_delete`, `rehydrate`. **Análisis:** extraer firmas + parámetros reales de las 8 tools. **Implementación:** documentar las 8 en MCP.md + gate de paridad tool↔doc. | `docs/api/MCP.md`, `vantadb-mcp/src/lib.rs` | 🟡 2-4h | 🟡 | ⏳ Pendiente |
| `AUD-007` | **Drift ARCHITECTURE.md vs código** — **Investigación:** ef_construction=400 vs real 100 (`src/index/graph.rs:259`); `HnswIndex`/`src/storage/wal.rs` no existen (es `CPIndex`/`src/wal.rs`); `WalSharded` vs real `ShardedWal`. **Análisis:** confirmar cada drift contra código. **Implementación:** corregir ARCHITECTURE.md con nombres y constantes reales. | `docs/architecture/ARCHITECTURE.md`, `src/index/graph.rs`, `src/wal.rs` | 🟢 1-2h | 🟡 | ⏳ Pendiente |
...
| `AUD-011` | **Deuda unsafe/unwrap/OpGate no uniforme** — **Investigación:** 86 `unsafe` en 16 archivos (top: `index/distance.rs` 23, `storage/vfile.rs` 22; 0 en bindings); 1904 unwraps/expects en 62.7K LOC (~1/33 líneas); pánico confirmado `src/storage/engine/ops.rs:1761`; patrón `OpGate`+`drain()` solo en `vantadb-node/src/lib.rs:75-92,277-287` (falta en python/wasm → riesgo write-after-close). **Análisis:** priorizar pánicos en hot path + portar OpGate a python/wasm bindings. **Implementación:** reemplazar unwraps críticos por propagación de error + añadir gate de cierre en bindings restantes. | `src/storage/engine/ops.rs`, `vantadb-python/src/lib.rs`, `vantadb-wasm/src/lib.rs` | 🟡 1-2d | 🔴 | ⏳ Pendiente |
