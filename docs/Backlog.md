...
| `UX-05` | 🟡 | **Token `.label-tech`**: `font-tech text-[10px] uppercase tracking-widest` repetido 15+ veces en el shell — extraer clase compartida en index.css. *Utility `@utility label-tech` aplicada y sobrescrito el replace masivo — re-aplicar el reemplazo en ~20 archivos* | `WorkspaceShell.tsx` (~15 usos), `index.css` | ⬜ Pendiente |
