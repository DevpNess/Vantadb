...
| `AUDIT-03` | **🛡️ Miri guard en el boundary PyO3/NumPy** - Ejecutar `cargo +nightly miri test` con `MIRIFLAGS=-Zmiri-tree-borrows` sobre `vantadb-python` (path `try_numpy_array` / `__array_interface__`) antes de cualquier cambio FFI adicional. Detectar UB de alias/lifetime no cubierto por tests unitarios. **Gate recomendado previo al fix AUDIT-01 y a futuros cambios de superficie FFI.** *(hallazgo audit-full 2026-08-04, rec. 2)* | `vantadb-python/src/convert.rs`, `vantadb-python/src/vector.rs`, `vantadb-python/src/types.rs` | 🟢 2-4h | 🔴 Bloqueante |
...
| `AUDIT-06` | **⚡ RRF fusion: skip single-channel** - `fuse_rrf_many`/`apply_rrf_contributions` construyen `BTreeMap<(String,String),Hit>` (2 String clones por candidato) incluso en paths single-channel (sparse puro, dense puro) o ya-uniques. Retornar el listado ordenado directo cuando hay 1 solo canal; usar el map solo al intercalar ≥2 canales. *(hallazgo audit-full 2026-08-04, F3 — downgraded a low por ISO review, ≤750 candidates)* | `src/planner.rs:144-185` | 🟢 1-2h | 🔵 Baja |
| `AUDIT-07` | **🔍 Evaluar SparseVector `BTreeMap<u32,f32>` vs sorted `Vec<(u32,f32)>`** - BTreeMap es correcto/determinístico (dot merge-lineal eficiente) pero mayor footprint por entrada + JSON más grande. Si los sparse vectors crecen, el Vec ordenado da mejor localidad de `dot` y menor serialización. **Investigación primero — decidir solo si el corpus sparse escala.** *(hallazgo audit-full 2026-08-04, F5)* | `src/node.rs:409` | 🟢 2-4h | 🔵 Baja |
