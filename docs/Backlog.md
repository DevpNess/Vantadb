...
| `R2` | **Crear agente `vanta-research`** — convertir `research-agent.md` en agente real (mode: subagent, tools read-only + web: codegraph, metasearchmcp, argus, read/grep/glob; skills: coordinated-web-search, source-driven-development, progreso) para que el lead delegue discovery y reciba digest ≤500 palabras sin gastar contexto. | `.opencode/agents/vanta-research.md` (nuevo) | 🟢 | 🔴 | 📝 Pendiente |
...
| `R7` | **Corregir comandos de verificación rotos en Output Templates** — `vanta-worker.md:102` `cargo check -p vantadb-python` → `cargo check -p vantadb_py` (package real en `vantadb-python/Cargo.toml:2`; learning AUD-039: "-p vantadb-python did not match any packages"); `vanta-docs.md:102` `pytest vantadb-python/tests/` → `target/audit-venv/Scripts/python -m pytest vantadb-python/tests/test_sdk.py`. Checks que "pasan" sin correr = peor que no tenerlos. | `.opencode/agents/vanta-worker.md`, `.opencode/agents/vanta-docs.md` | 🟢 | 🔴 | 📝 Pendiente |
