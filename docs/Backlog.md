...
| ID | Tarea | Archivo | Esfuerzo | Prioridad | Estado |
|----|-------|---------|----------|-----------|--------|
| `DRV-016` | **Inconsistencia de Mutex: `governor.rs` usa `std::sync::Mutex` en vez de `parking_lot`** — Usa std Mutex + `MutexExt` (auto-unwrap). 💡 Cosmético: poison solo ocurre si hay panic en sección crítica. 🔵 estilo, no bug. Mantener como deuda de consistencia | `src/vector/governor.rs:94,111,147,160` | 🟢 30min | 🔵 | ℹ️ |
...
| `DRV-031` | **Comentario doc duplicado** — `/// Put or update a namespace-scoped persistent memory record.` aparece dos veces (L1140-1142) | `vantadb-python/src/lib.rs:1140-1142` | 🟢 2min | ℹ️ | ❌ SKIP: bug ya no existe (el doc existe 1 vez, refactor posterior lo limpió) |
...
| ~~`DRV-040`~~ | **`unsafe` en simd.rs sin `// SAFETY:` comment** — ❌ SKIP: `vantadb-wasm/src/simd.rs` no existe en el codebase. No se encontró archivo simd.rs en ningún directorio. Referencia eliminada | ~~`vantadb-wasm/src/simd.rs:34-77`~~ | 🟢 30min | 🟡 | ✅ |
...
| ~~`DRV-116`~~ | **10 warnings de compilación en `vantadb` core** — ✅ `cargo check -p vantadb` y `-p vantadb-mcp` producen ZERO warnings. Verificado Jul 23. Side effect de refactors previos | `—` | 🟢 30min | ⚪ | ✅ |
