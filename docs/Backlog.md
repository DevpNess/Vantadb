...
> **Total open items:** 105 activas (−FIND-58 cerrada 2026-09-03 gate wasm32 crudo verde; −GOV-TK1 cerrada 2026-09-03 restore --dry-run, ambas mitades completas; −FIND-57 cerrada 2026-09-03; −FIND-59 cerrada 2026-09-04 análisis (d); +FIND-61 spike medición (neto 105); post-campana quality-gtm-wave 2026-09-03: -11 filas por ejecucion RES-07/03/12/15C/09 + GOV-TK1/9 + MKT-18h/f/i + SRV-07 + AUD-045, +2 FIND-56/58 nuevos (FIND-57 cerrada), GOV-TK1/MKT-18f re-escaladas; split negocio: 15 filas a Backlog-negocio.md; previo 126)
---
...
| FIND-61 | Baja | **Spike medición insert_lock vs fsync (follow-up FIND-59/ADR-037, timebox ≤1d, 0 código prod):** el análisis no pudo cuantificar el desglose por-op sin medir. Medir con harness existente: `SyncMode::Always` (baseline §13 ~111.5 ops/s) vs `SyncMode::Never` (solo lock+HNSW) vs prototype micro-batching en pipeline (N={8,16,32}, bench-only flag; reportar ops/s + p50-ack-latency + ventana de pérdida) + verificar cobertura ERR-010 del path `commit_transaction` (WAL batch + apply SIN `insert_lock`, `txn.rs:119-213`). Gate: abrir slice de implementación SOLO si batch ≥2× sobre §13 Y FUT-12 decidió la política de ventana de pérdida; si no, cerrar con números. No tocar sin bench A/B contra §13 (Regla 9) | `benches/ingestion_concurrent.rs`, `src/storage/engine/txn.rs`, BENCHMARKS §13, ADR-037 | 🟢 spike ≤1d | 🟢 Baja | ⬜ Pendiente |
