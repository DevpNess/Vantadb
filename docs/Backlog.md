...
| `MEM-39` | **Hueco triage P27: seed/import CLI** — TDAM `core/seed/*` (input.ts 433L + seed-runtime.ts 369L) fue triajeado como referencia (research 09:37,50) pero ninguna tarea lo agendó. Comando de import/seed inicial para vanta-memory (skills/persona iniciales). Detectado en auditoría post-campaña 2026-08-21. | `vanta-memory/src/seed/`, `src/cli.rs` | 🟡 | 🟡 | ❌ Pendiente |
| `MEM-40` | **Hueco triage P27: recall cross-sesión** — `auto_recall.rs` busca solo por `session_key`; TDAM auto-recall.ts busca global-scope con isolation. Decidir: ¿cross-sesión deseable para VantaDB? Si sí, implementar con filtro isolation; si no, documentar decisión + ADR. Detectado en auditoría post-campaña 2026-08-21 (confianza media — verificar contra TS antes). | `vanta-memory/src/core/hooks/auto_recall.rs`, `docs/architecture/adr/` | 🟡 | 🟢 | ❌ Pendiente |
| `MEM-41` | **Hueco triage P27: generation-log provenance** — TDAM `core/memory-generation-log/*` (189L): log consultable de provenance de generaciones L1/L2/L3 (layer/status/anchor). MEM-34 telemetría cubre métricas pero no provenance consultable. Valor medio; evaluar si vale antes de F5. Detectado en auditoría post-campaña 2026-08-21. | `vanta-memory/src/core/memory_generation_log/` | 🟡 | 🟢 | ❌ Pendiente |
