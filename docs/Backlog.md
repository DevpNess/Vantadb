...
| `FIND-27` | 🔴 | Bug candidato descubierto en FIND-02: provider Ollama nativo postea a `/api/embeddings` (endpoint legacy, campo `prompt`) usando campo `input` → falla contra Ollama actual. API vigente: `POST /api/embed` `{model,input}` → `embeddings[0]`. Afecta auto-embedding del executor. Fix + test contra server real o mock | `src/llm.rs:100` · hallazgo FIND-02 2026-08-23 | ❌ Pendiente |
