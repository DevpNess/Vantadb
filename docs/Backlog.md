...
| `GH-143` | **ci: Acelerar CI con sccache y paralelización** — El pipeline CI corre build/test/clippy pero puede optimizarse. **Task:** (1) habilitar `sccache` para cachear compilación Rust, (2) evaluar reemplazar `cargo build` por `cargo check` donde baste, (3) paralelizar jobs independientes. **DoD:** CI ≥20% más rápido; todos los checks existentes pasan. **Cierre:** revisar y cerrar issue #143. Relacionado con `INV-017` (sccache investigación). | `.github/workflows/*.yml` | 🟡 2-4h | 🟡 | ❌ Desde cero |
| `GH-142` | **ci: Smoke tests de examples en CI** — Existen `examples/python/` y `examples/rust/` sin verificación en CI. **Task:** agregar job que (1) corra cada ejemplo Python (smoke, no benchmark), (2) `cargo run --example` por ejemplo Rust, (3) reporte claro de fallos. **DoD:** CI corre smoke tests de examples; todos pasan; CI bloquea PRs que rompen examples. **Cierre:** revisar y cerrar issue #142. | `.github/workflows/` (nuevo job) | 🟡 2-4h | 🟠 | ❌ Desde cero |
...
| `GH-124` | **docs: Ejemplos doc-test para API pública Rust** — El crate core tiene ejemplos públicos mínimos; `cargo doc` debe mostrar ejemplos runnable. **Task:** agregar doc-test a `VantaEmbedded::open()`/`open_with_config()`, `put()`, `get()`, `search()`, `delete()`, y campos de `VantaConfig`. **DoD:** `cargo doc --no-deps` sin warnings; doc-tests pasan con `cargo test --doc`; ejemplos realistas y ejecutables. **Cierre:** revisar y cerrar issue #124. | `src/lib.rs` o módulos relevantes | 🟡 3-5h | 🟡 | ❌ Desde cero |
...
| `GH-122` | **docs: Docstrings en API pública del Python SDK** — El SDK (`vantadb-py`) tiene métodos sin docstrings; usuarios dependen de `help()` y autocomplete. **Task:** docstrings completos en `vantadb-python/src/lib.rs` para `VantaDB.new()`, `put()`, `get_memory()`, `delete_memory()`, `list_memory()`, `search_memory()`, `insert()`, `rebuild_index()`, `export_namespace()`, `export_all()`, `import_file()`, `operational_metrics()`. Cada uno: Args, Returns, Raises, ejemplo de uso. **DoD:** todos los métodos públicos con docstrings (visibles como docstrings Python vía PyO3); ejemplos runnable. **Cierre:** revisar y cerrar issue #122. | `vantadb-python/src/lib.rs` | 🟡 3-5h | 🟡 | ❌ Desde cero |
