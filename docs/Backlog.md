...
| FIND-52 | Alta | **Regresión runtime wasm32 + ICE release (origen: verificación ERR-TS-01 2026-09-02):** con pkg reconstruido desde HEAD, 30/278 tests de `vantadb-ts` fallan por panics `std::sys::time::unsupported` y `std::sys::sync::condvar::no_threads` (rutas del grafo de features `wasm` llaman `std::time`/`Condvar` — requieren shim `web_time`/`parking_lot` cfg-gated); además `wasm-pack build --release` aborta rustc con STATUS_STACK_BUFFER_OVERRUN (1.95.0 MSVC, opt-level=s+cg1, persiste con cg16) — solo `--dev` compila localmente; el pkg prebuilt 29/8 tampoco carga ya en Node 24/26 (`vantadb_new` undefined, glue bundler viejo). Evidencia: reproducible con `wasm-pack build --dev --target bundler` + `npx vitest run` en `vantadb-ts` (226 pass / 30 fail); proof-stash reproduce persistence=false en HEAD sin diff ERR-TS-01 (open() `persistence:false` = WSM-01, test hardening:401 stale). Fix: auditar callers std::time/Condvar bajo feature `wasm`, actualizar test stale, workaround ICE (perfila wasm32 dedicada o cg16+opt=2). Colateral detectado: rebuild napi de `vantadb-node` regenera `index.d.ts` con +44L (VacuumReport/RebuildReport…) vs la versión commiteada — BND-10 dejó el d.ts fuera de sync; commitear regeneración en su follow-up (revocado en c68e2dca para mantener scope ERR-TS-01). | `vantadb-wasm/src/lib.rs`, `src/**` (feature wasm), `vantadb-ts/src/__tests__/hardening.test.ts`, `vantadb-node/index.d.ts` | 🔴 | 🔴 Alta | ⬜ Pendiente |
| FIND-53 | Media | **Métricas de error sin crate `metrics` (origen: ERR-OBS-01 2026-09-02):** el counter `vantadb_errors_total{code}` del contrato de observabilidad NO se cableó porque el crate externo `metrics` no es dependencia del crate raíz (solo existe el registry in-tree `src/metrics/`). ERR-OBS-01 prohíbe agregar deps sin justificar. Fix: evaluar `metrics` 0.24+ vs extender `src/metrics/core/registry.rs` (¿exporter Prometheus ya sirve?), luego `metrics::counter!("vantadb_errors_total", "code" => e.code())` en `vanta_error_response`/`query_error_response` (cardinality=10 codes, acotada). Por ahora: tasas de error derivables de logs estructurados `error.code=` (nivel WARN/ERROR) — ver `docs/operations/OBSERVABILITY.md` §4–§5. | `Cargo.toml`, `src/server/errors.rs`, `src/metrics/` | 🟢 1-2h | 🟡 Media | ⬜ Pendiente |
| FIND-54 | Baja | **Test pre-existente fallando bajo `--features server` (origen: verificación ERR-OBS-01 2026-09-02):** `server::router::tests::cors_layer_none_when_empty` falla determinístico en HEAD (sin diffs): `cors_layer(&[""])` retorna `Some` porque `HeaderValue::from_str("")` es válido en http 1.x y `filter_map` no descarta origins vacíos → el assert `.is_none()` se rompe. No corre en CI porque el perfil audit no habilita `server`. Fix trivial: `filter(|o| !o.is_empty())` antes del collect + test sibling. Evidencia: `cargo test -p vantadb --lib --features server server::router::tests::cors_layer_none_when_empty -- --exact` → panic en `src/server/router.rs:378`. | `src/server/router.rs` | 🟢 15m | 🟢 Baja | ⬜ Pendiente |
| FIND-55 | Media | **Sanitizar body 500 de `query_error_response` (origen: NOTICIED de ERR-OBS-01 2026-09-02):** los 5xx (Io/Storage/generic) devuelven `data: "Execution Error: {e}"` con el Display interno del motor filtrándose al cliente — patrón que `panic_error_response` ya mitiga (AUDREP-32) y que la regla de mapeo por capa recomienda (log full chain server-side, respuesta genérica al caller). NO se corrigió en ERR-OBS-01 porque el contrato era de observabilidad y consumidores TS regex-mirroran prefijos de Display. Fix: en rama 5xx de `query_error_response`/`vanta_error_response`, responder `"internal error (see code)"` con `code` VANTADB_* en el JSON (ya existe el campo post-ERR-CORE-01) y dejar `tracing::error!` con chain completa; los 4xx mantienen message descriptivo (son datos del input del usuario). Verificar que `classifyWasmError`/tests no dependan de Display en 5xx. | `src/server/errors.rs`, `vantadb-ts/src/errors.ts` (solo verificación) | 🟢 30m-1h | 🟡 Media | ⬜ Pendiente |
