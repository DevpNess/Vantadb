...
| `VS-CORE-01` | **Cursor/paginación en el bridge desktop (re-scopeado)** — el core YA tiene cursor (`list` con `options.cursor` + `next_cursor` en `api.rs:545`, expuesto en Python/TS/WASM); gap real solo en Desktop: comando Tauri `vanta_list` + `vanta.ts` no devuelven cursor. Exponer aditivo. → destraba virtualización real (VS-05). Gap §8.1. | `desktop/src-tauri/src/commands/data.rs:65`, `desktop/src/vanta.ts:159`, `src/sdk/api.rs:545` | 🟡 | 🔴 | ❌ Pendiente |
| `VS-CORE-02` | **Contadores por namespace + stats TTL** — método `{namespace: {count, expiring_soon, expired}}` reutilizando `count`/scan (hoy = N llamadas paginadas). → sidebar + HOME (VS-04). Gap §8.3. | `src/sdk/api.rs` (`count` :1278), `src/metrics/core/snapshot.rs:41` (50 campos) | 🟢 | 🔴 | ❌ Pendiente |
| `VS-CORE-03` | **`explain` estructurado: ya existe → consumir (re-scopeado)** — `VantaSearchExplanation` (`types.rs:418` con `fusion_report` + `rrf_k`) y `VantaSearchExplanationHit` (`types.rs:429` con `bm25_terms`/`rrf_*_rank`) ya existen y están expuestos (Rust `explain_memory_search`, Python, TS `explainSearch`). **NO implementar nuevo**: Fase 1 consume lo existente; opcional añadir vector score crudo (hoy solo rank). Gap §8.2 ya resuelto. | `src/sdk/types.rs:418,429`, `vantadb-ts/vantadb.ts:435` | 🟢 | 🟠 | ❌ Pendiente (Fase 1, re-scopeado) |
| `VS-CORE-04` | **Exportar selección/query** (no solo namespace) a JSONL/`.vdbdump` — añadir `filter: Option<VantaMemoryFilter>` a `export_namespace` (internamente `records_for_namespace` :76 ya acepta filtros). → P12/P13. Gap §8.4. | `src/sdk/serialization/impl_export.rs:121` | 🟢 | 🟠 | ❌ Pendiente (Fase 2) |
| `VS-CORE-05` | **Batch delete con filtro desde UI** — `delete_by_filter` existe en core (`api.rs:1210`) pero NO en Python/WASM/TS/`vanta.ts`; exponer en WASM → `vanta.ts` + comando Tauri `vanta_delete_by_filter` con confirmación + undo. Gap §8.6. | `desktop/src/vanta.ts`, `src/sdk/api.rs:1210` | 🟢 | 🟠 | ❌ Pendiente (Fase 2) |
| `VS-CORE-06` | **IQL en desktop: exponer en bridge + autocompletado (re-scopeado)** — `query` ya expuesto en nativo (`api.rs:1110`), WASM (`vantadb-wasm/src/lib.rs:1210`) y TS (`vantadb.ts:680`); falta: comando Tauri `vanta_query` + shim de autocompletado core-side (prefix sobre `parse_statement`, crate-interno). → consola IQL (lente GRAFO). Gap §8.5 ya resuelto en bindings. | `src/parser.rs::parse_statement`, `desktop/src/vanta.ts` | 🟢 | 🟠 | ❌ Pendiente (Fase 2, re-scopeado) |
| `VS-CORE-07` | **Retención de versiones históricas en `VantaMemoryRecord`** — planificación → investigación → análisis → implementación (D2). Decisión de core: retener snapshots n-1 vs solo `version` actual; coste de almacenamiento; API de acceso. **Destraba Historial+Diff (Fix 3, Fase 1) que queda EN ESPERA.** Gap §8.7. | `src/sdk/types.rs`, `src/sdk/api.rs` | 🟡 | 🟠 | ❌ Pendiente |
...
| ID | Descripción (→ Resultado) | Archivos | Esfuerzo | Prio | Estado |
|----|-------------|----------|----------|------|--------|
| `MEM-01` | **F1 Search profile por namespace en core** — parametrizar `fuse_rrf`/`hybrid_candidate_budget` de `planner.rs` (hoy constantes globales RRF_K=60, CANDIDATE_MULTIPLIER=4, clamp 32/256) + campos `mode: keyword\|vector\|hybrid`, `rrf_k`, `candidate_k` en `VantaMemorySearchRequest` con serde defaults retrocompatibles. → búsqueda configurable por namespace. | `src/planner.rs`, `src/sdk/serialization/vector_types.rs`, `src/sdk/types.rs`, `src/sdk/search/mod.rs` | 🟡 | 🔴 | ❌ Pendiente |
| `MEM-02` | **F1 Exponer search profile en MCP/search** — parámetro opcional `search_profile` (mode/rrf_k/candidate_k) en el tool MCP de búsqueda → passthrough a `VantaMemorySearchRequest`; sin profile → defaults de MEM-01. → agentes controlan recall/precisión. | `vantadb-mcp/src/handlers/tools.rs`, `vantadb-mcp/src/validation.rs`, `vantadb-mcp/src/config.rs` | 🟢 | 🔴 | ❌ Pendiente |
| `MEM-03` | **F2 Entidades `entity_*` + CRUD en core** — modelo teams/users/agents/tasks/assets (hoy NO existe; modelo actual namespace+key): `UserEntity{user_type}`, `TeamEntity{owner_user_id, TeamRole}`, `AssetEntity{AssetType, AssetVisibility=private/team/restricted/agent/task, AssetStatus}`, `InjectionMode`; CRUD mínimo; protección `last_system_admin` (409). Decisión pendiente: tablas propias vs nodos partición InternalMetadata (patrón thread.rs). | `src/entity.rs` (nuevo), `src/entity/tests.rs`, `src/lib.rs` | 🟡 | 🔴 | ❌ Pendiente |
| `MEM-04` | **F2 Permission-checker allow-only** — cadena `resource→owner→member→visibility→role-default→ACL→deny` (96 líneas reales, allow-only: `effect=deny` NO deniega); `visibility=restricted` evalúa ACL inline antes de role-default; sysadmin no bypassa; recurso inexistente → `asset_not_available` anti-enumeración. ⚠️ Contradicción SYNTHESIS (96 líneas) vs 06 §6 (~40 líneas) a resolver. `src/rbac.rs` es dead code — decidir reemplazo vs coexistencia. | `src/permissions.rs` (nuevo) o `src/entity/checker.rs`, `src/lib.rs`, `src/rbac.rs` | 🟡 | 🔴 | ❌ Pendiente |
| `MEM-05` | **F2 Auth 3 capas en server** — L1 Bearer con `timingSafeEqual`; L2 header service-id → 401 si falta; L3 user-key → userId/isSystemAdmin desde entity_*; extiende `AuthState`+`auth_middleware` existentes (`cli_server.rs:275-351`); `/health` pública. Quota/billing DIFERIDO (SYNTHESIS §3). | `src/cli_server.rs`, `vantadb-server/src/middleware.rs`, `vantadb-server/src/server.rs` | 🟢 | 🔴 | ❌ Pendiente |
| `MEM-06` | **F3 Esquema skills multi-versión en core** — VantaDB no es SQL: namespace `skills` + nodos por versión con metadata (`version`, `is_head`, `content_hash`, `expires_at`, owner_agent), índice único parcial por `(owner,name)` donde is_head, optimistic lock `expected_version`, TTL keep-recent=3, idempotencia por content-hash; CRUD create/update/patch/delete/versions. Reusar text_index + HNSW propios (NO vec0). | `src/skills.rs` (crear), `src/sdk/types.rs`, `src/lib.rs`, `tests/` | 🟡 | 🔴 | ❌ Pendiente |
| `MEM-07` | **F3 MCP tools skill_*** — 6 tools del review agent (`skill_list/view/create/update/patch/files_write`) sobre MEM-06; writes exigen `expected_version`; owner check → 404 sin filtrar existencia; límites 5MB/recurso, 50MB/skill. | `vantadb-mcp/src/handlers/tools.rs`, `vantadb-mcp/src/skills.rs` (crear) | 🟡 | 🔴 | ❌ Pendiente |
| `MEM-08a` | **F4 vanta-memory scaffolding** — crate nuevo `vanta-memory/` como workspace member (fuera de default-members como mcp/server/wasm); `lib.rs` + tipos base serde round-trip; sin lógica aún. | `vanta-memory/Cargo.toml`, `vanta-memory/src/lib.rs`, `Cargo.toml` raíz | 🟢 | 🔴 | ❌ Pendiente |
| `MEM-08b` | **F4 Contratos L1 + trait LLMRunner** — `MemoryRecord{id,content,type,priority,scene_name,source_message_ids,metadata,timestamps,version,sessionKey}`, `DedupDecision{record_id,action,target_ids,merged_*}`, `L1ExtractionResult` (01 §5); trait host-neutral `LLMRunner.run({prompt,systemPrompt,taskId,timeoutMs,workspaceDir,storage,tools})` — VantaDB NO tiene LLM generativo (`src/llm.rs` solo embeddings). ⚠️ Decisión: sync vs async condiciona MEM-09..21. | `vanta-memory/src/types.rs`, `vanta-memory/src/llm.rs` (crear) | 🟡 | 🔴 | ❌ Pendiente |
| `MEM-09` | **F4 L0 auto-capture** — hook `auto_capture(thread_id, message)` persiste vía `ThreadStore::append` + notifica pipeline; idempotente por mensaje; captura TODO (filtro vive en L1); sync usable desde WASM. | `src/agentic/thread.rs`, `vanta-memory/src/l0/capture.rs` (crear), `src/agentic/mod.rs` | 🟢 | 🔴 | ❌ Pendiente |
| `MEM-10` | **F4 L1 extracción LLM** — `shouldExtractL1` (largo/símbolos/prompt injection), split new(10)+background(5), 1 call LLM JSON → `SceneSegment{scene_name,message_ids,memories[]}`, parsing con reparación (`sanitizeJsonForParse`), tipos persona/episodic/instruction/work_*/conocimiento con prioridad 0-100. Prompts reescritos (NO traducir Kenty chino). | `vanta-memory/src/l1/extractor.rs`, `l1/prompts.rs`, `l1/types.rs` (crear) | 🟡 | 🔴 | ❌ Pendiente |
| `MEM-11` | **F4 L1 dedup 2 fases** — fase 1 recall candidatos (vector topK 5, degradación BM25, sin capacidad → store-all sin error); fase 2 juicio LLM batch → `store\|update\|merge\|skip` con `target_ids`; parse fail → store-all (nunca pierde data); IDs `m_<ts>_<hex>`. | `vanta-memory/src/l1/dedup.rs`, `l1/prompts_dedup.rs` (crear) | 🟡 | 🔴 | ❌ Pendiente |
| `MEM-12` | **F4 Nodo escena con contrato META en graphrag** (opcional — SYNTHESIS §2.1.3) — tipo de nodo escena `{summary,heat,created,updated}` + edge L1→escena por `scene_name`; LLM-free, WASM-safe; ordenable por heat desc. Primera descartable si el alcance aprieta. | `src/graphrag/scene.rs` (crear) o `src/graphrag/pipeline.rs`, `src/graphrag/mod.rs` | 🟢 | 🟠 | ❌ Pendiente |
| `MEM-13` | **F4 L2 SceneExtractor — tools sandbox + store** — tools read/write/edit sandboxed con allowlist y path validation (sin exec/sessions/browser/cron); store de escenas; backup/restore ante fallo LLM. | `vanta-memory/src/l2/scene_store.rs`, `l2/tools.rs` (crear) | 🟡 | 🔴 | ❌ Pendiente |
| `MEM-14` | **F4 L2 SceneExtractor — extractor + prompts + cleanup** — agente LLM con strategy UPDATE>MERGE>CREATE (máx 1 nuevo/batch), `maxScenes=15` warnings escalonados, soft-delete `[DELETED]`, heat CREATE=1/UPDATE=+1/MERGE=sum+1, normalización filenames, `emptyExtraction` si LLM falla. ⚠️ Heat lo mantiene el LLM (02 §8). | `vanta-memory/src/l2/scene_extractor.rs`, `l2/prompts.rs` (crear) | 🟡 | 🔴 | ❌ Pendiente |
| `MEM-15` | **F4 L3 PersonaGenerator** — filtro escenas cambiadas por checkpoint (skip si sin cambios), modo `first\|incremental`, límites 2000/1200 chars, `escapeXmlTags` anti-inyección, navegación por heat; 5 triggers por prioridad (request explícito, cold start, recovery, primera escena, `triggerEveryN=50`). | `vanta-memory/src/l3/persona_generator.rs`, `l3/prompts.rs`, `l3/triggers.rs`, `l3/navigation.rs` (crear) | 🟡 | 🔴 | ❌ Pendiente |
| `MEM-16` | **F4 Consolidación por triggers** — orquestación server (no fase propia — SYNTHESIS fila 6): timers per-session + locks por grano (L1 session / L2-L3 agent), encadenado L0→L1→L2→L3, `StatefulPipelineManager` sin Redis (estado local/checkpoint), warmup 1→2→4→5, reloj fake para tests. | `vanta-memory/src/pipeline/manager.rs`, `pipeline/state.rs` (crear), opcional `vantadb-server/src/` | 🟡 | 🔴 | ❌ Pendiente |
| `MEM-17` | **F4 Skill extracción LLM** — transcript con marcadores `<<past-user>>` anti role-capture, truncado head 8k/tail 32k, pre-carga de skills propios anti-duplicados, review prompt taxonomía SOP/Background/Preference, tool set SIN delete/files_remove, sink idempotente. Síncrono o cola local (decidir). | `vanta-memory/src/skill/extractor.rs`, `skill/prompts.rs` (crear) | 🟡 | 🔴 | ❌ Pendiente |
| `MEM-18` | **F4 Patrón recall — prepend/append + 3 modos** — `assemble_recall(user_text, memories, persona, scenes, budget) → {prepend, append}`: prependContext dinámico (memorias) vs appendSystemContext cacheable (persona/escenas) para no romper prompt caching; modos keyword/vector/hybrid delegando en MEM-01; hybrid sin vector → degrada keyword. | `vanta-memory/src/recall.rs` (crear) | 🟢 | 🔴 | ❌ Pendiente |
| `MEM-19` | **F4 Patrón recall — sanitizeText + budget code-point** — `sanitize_text` anti feedback-loop (quita `<relevant-memories>`, `<user-persona>`, `<scene-navigation>`, timestamps, base64, directivas); truncación por code-point (nunca partir surrogate pairs); `max_chars_per_memory` + `max_total_recall_chars`. | `vanta-memory/src/sanitize.rs`, `vanta-memory/src/budget.rs` (crear) | 🟢 | 🔴 | ❌ Pendiente |
| `MEM-20` | **Core Cursor `lastOffloadedToolCallId` por sesión** — marca persistente de hasta dónde comprimió el offload; compresión ignora mensajes ≤ cursor; avanza solo tras confirmar; idempotente ante retry. LLM-free, WASM-safe. | `src/session_state.rs` (nuevo) o `src/agentic/thread.rs`, `src/lib.rs` | 🟢 | 🔴 | ❌ Pendiente |
| `MEM-21` | **F4 MCP tools scene_*** — `scene_read` (guard anti path-traversal), `scene_list` ordenado por heat desc, `scene_query` por nombre — sobre nodos MEM-12 + heat MEM-14. | `vantadb-mcp/src/handlers/tools.rs`, `vantadb-mcp/src/scenes.rs` (crear) | 🟢 | 🟠 | ❌ Pendiente |
| `MEM-22` | **F5 Context Engine offload — assemble + cascada** — `assemble(msgs, ratio) → {messages, report}`; `ratio<0.5` → skip; cascada mild (`compressByScoreCascade`, MIN=10/INITIAL=7/FLOOR=1, **revertir si summaryLength > originalLength**), aggressive one-shot con fingerprint (role + 200 chars), no partir pares tool_call/tool_result. | `vanta-memory/src/context_engine.rs`, `compressor.rs` (crear) | 🟡 | 🔴 | ❌ Pendiente |
| `MEM-23` | **F5 Context Engine — emergency + token estimator + report** — emergency trunca a ~2000 chars sin partir tool_call_pair; estimador `estimate_tokens(text) ≈ chars/3` configurable sin deps (⚠️ decidir: 3 chars vs tiktoken o200k_base); `report` expone modo/msgs conservados/tokens antes-después. | `vanta-memory/src/token_estimator.rs` (crear) | 🟢 | 🔴 | ❌ Pendiente |
| `MEM-24` | **F5 MMD como memoria de tarea** — persistir MMD por sesión (partición InternalMetadata patrón thread.rs); ACTIVE siempre, HISTORY solo post-aggressive; dedup por fingerprint `${len}:${slice(0,64)}`; marker `_mmdContextMessage`; no partir pares tool_call. ⚠️ Formato: Mermaid literal (05) vs contrato META (propuesta VantaDB) — decidir. | `vanta-memory/src/mmd.rs`, `mmd_injector.rs` (crear) | 🟡 | 🟠 | ❌ Pendiente |
| `MEM-25` | **F6 vanta-proxy crate + 3 protocolos wire** — binario opcional `vanta-proxy/` (⚠️ decisión: crate aparte vs feature de vanta-server; puertos propios); interceptación verbatim OpenAI Chat Completions / Anthropic Messages / Responses API con rutas `/:agent/:spaceId/v1/...`; forward timeout 600s, upstream caído → 502; SIN agent-adapters. | `vanta-proxy/Cargo.toml` (nuevo), `vanta-proxy/src/main.rs`, `server.rs`, `handlers/{openai,anthropic,responses}.rs`, `Cargo.toml` | 🟡 | 🟠 | ❌ Pendiente |
| `MEM-26` | **F6 vanta-proxy — ciclo auth→session→injection** — auth contra RBAC local (no Gateway remoto TDAM); sessionKey desde headers (`x-conversation-id\|x-session-id\|x-claude-code-session-id\|x-chat-id\|x-thread-id`); state machine local team→agent→task con TTL 30min solo pending; inyección L2/L3 en system prompt, L0/L1 como tools (no invalidar KV-cache); SIN `/v3/session/init` externo. | `vanta-proxy/src/auth.rs`, `session.rs`, `inject.rs` (crear) | 🟡 | 🟠 | ❌ Pendiente |
| `MEM-27` | **F6 vanta-proxy — rate-limit + write-back + reporting + mem-command** — sliding window in-process por `spaceId×model` con 429/Retry-After, fail-open consciente sin Redis; write-back fire-and-forget retry 3 backoff 500ms→1s→2s + flush SIGTERM (write-back = memory_put local); reporting log por turno sin Opik/Langfuse; `mem:` whitelist deshabilitada por defecto. | `vanta-proxy/src/rate_limit.rs`, `writeback.rs`, `mem_command.rs`, `report.rs`, `config.rs` (crear) | 🟡 | 🟠 | ❌ Pendiente |
| `MEM-28` | **F7 Wiki store + state machine pending→ready** — módulo `src/wiki/` LLM-free: estados `pending→processing(scanning/ingesting)→ready\|failed(+sync_error 500 chars)`, `run_id` por build, re-ingest 409 si pending/processing, frontmatter `locked:true` con borrado en cascada, dedup por path canónico type+title. | `src/wiki/mod.rs`, `src/wiki/store.rs`, `src/wiki/state.rs`, `src/lib.rs` | 🟡 | 🟠 | ❌ Pendiente |
| `MEM-29` | **F7 Fetcher SSRF + chunker 12k/400** — fetch https-only con SSRF blocklist (10./172.16-31./192.168./169.254./127./0./localhost/::1/fe80:) y validación de IPs post-DNS; chunker defaults 12000/400 chars, `SOURCE_CHAR_BUDGET=28000`; upload `raw/*` sin fuentes → error; SSRF check no desactivable. ⚠️ Dep de red → feature-gate para WASM. | `src/wiki/fetcher.rs`, `src/wiki/chunker.rs` (crear) | 🟡 | 🟠 | ❌ Pendiente |
| `MEM-30` | **F7 Ingest merge serial + límite concurrencia LLM** — worker único (no 2 servicios): `commitCandidates` SERIAL, merge por página bajo `pLimit(5)` global (clamp 1-20), fallo por página no bloquea resto, `ensureSources` forzando frontmatter `sources`; merge LLM en vanta-memory (no core WASM). | `vanta-memory/src/ingest/` (nuevo), `src/wiki/`, `vantadb-server/src/` | 🟡 | 🟠 | ❌ Pendiente |
| `MEM-31` | **F7 Callback S2S con run_id** — callback de progreso con `run_id` que descarta paquetes tardíos (build anterior), throttle `PROGRESS_THROTTLE_MS=500`, summary ≤100 chars / ≤20 páginas. ⚠️ Destino: vanta-server o hook síncrono local (no Panel — decidir). | `vanta-memory/src/ingest/callback.rs` (crear), `vantadb-server/src/` | 🟢 | 🟠 | ❌ Pendiente |
| `MEM-32` | **F7 MCP tools code_* query-only** — 8 tools `code_search/explore/callers/callees/impact/node/status/files` sobre graphrag EXISTENTE + `src/graph.rs` (`bfs_traverse`/`dfs_traverse`/`topological_sort` ya existen — el costo es solo exponer, no construir); respetar dirección de arista; read-only sin red ni `@colbymchenry/codegraph`. | `vantadb-mcp/src/handlers/tools.rs`, `vantadb-mcp/src/code.rs` (crear) | 🟡 | 🟠 | ❌ Pendiente |
| `MEM-33` | **F7 MCP tools wiki_*** — 4 tools `wiki_search/read/list/graph` sobre MEM-28; `wiki_search` con text index BM25 propio (no FTS5); `wiki_graph` BFS multi-hop cap 200; fallan claro si wiki `pending` (no `ready`); read-only. | `vantadb-mcp/src/handlers/tools.rs`, `vantadb-mcp/src/wiki.rs` (crear) | 🟢 | 🟠 | ❌ Pendiente |
| `MEM-34` | **Core Telemetría por capa** — extender `operational_metrics_snapshot()` (`src/metrics/core/mod.rs:545`, hoy startup/wal/ann/text_index/hybrid) con latencias L1/L2/L3/recall/offload, `persona_length_before/after`, `persona_drift_ratio`, `recall_hit_count/top_score/strategy`; error-silent; exponer por `/metrics`. (Fusión de propuestas duplicadas de 2 agentes.) | `src/metrics/core/mod.rs`, `src/metrics/core/state.rs`, `src/cli_server.rs`, hooks en `vanta-memory/src/` | 🟢 | 🟡 | ❌ Pendiente |
| `MEM-35` | **Core Data plane de referencia en server** — rutas `POST /conversation/add` (01 §10, vía ThreadStore) y `/skill/listing` (03 §4, sobre MEM-06 o stub 501 si no existe) protegidas por auth existente; NO copiar `/v3/session/init` ni `/v3/knowledge/query` (no existen en TDAM — endpoints propios VantaDB). | `src/cli_server.rs`, `vantadb-server/src/server.rs`, `vantadb-server/tests/e2e.rs` | 🟢 | 🟡 | ❌ Pendiente |
| `MEM-36` | **SDK TS+Python — sub-clientes por dominio** — expone `client.memory.*`, `client.graph.*`, `client.conversation.*` (agrupación por dominio, NO tenancy TDAM) manteniendo 100% backward-compat de la API plana; `vantadb-ts` (WASM) + `vantadb-python` (re-exports). | `vantadb-ts/src/vantadb.ts`, `vantadb-ts/src/types.ts`, `vantadb-python/vantadb/__init__.py` | 🟡 | 🟡 | ❌ Pendiente |
