...
| `FIND-29` | 🟢 | Último punto de cast manual `from_raw_parts(u8*→f32*)` vivo sobre datos mmap en el index: `search/layer.rs:66-71, 234-237`. Sound hoy (INV-024: `vector_offset % 4 == 0` + `debug_assert_eq!(align_offset(4), 0)` en layer.rs:61-65) pero candidato a migrar al helper canónico `VectorRepresentations::as_f32_slice()` (align_to) para eliminar el último UB manual del index | `src/index/search/layer.rs:66-71,234-237` · hallazgo vanta-audit FIND-28 2026-08-24 | ❌ Pendiente |
| `FIND-30` | 🟢 | `unused variable: ns` en `cli_server.rs:1302` (closure `move |ns: String|` ignora `ns`): rompe `cargo clippy --workspace --all-targets -- -D warnings` bajo feature `server` (el workspace lo habilita vía dependientes). Pre-existente, ajeno a REVIEW-17 (subsistema server, dueño MOD-13). Fix mecánico 1-línea: `_ns` o quitar param | `src/cli_server.rs:1302` · hallazgo DISCOVERY REVIEW-17 2026-08-25 | ❌ Pendiente |
| `FIND-31` | 🟡 | `purge_expired` tras reopen falla con `ValidationError "text index df would go negative"` si un record expirado tiene payload indexado: `purge_expired` computa deltas de term-stats sobre el estado del text index que tras reopen no coincide (replay_write_node no reconstruye text index; rebuild solo en open de HNSW). Reproducido con put→flush→reopen→purge (test aislado). Pre-existente, NO causado por MOD-04 (falla igual con código original); fix requiere reconstruir text index en reopen o guard de deltas en purge | `src/sdk/api.rs:1017-1038` (term_deltas), `src/storage/engine/init.rs` recover_state · hallazgo DISCOVERY MOD-04 2026-08-25 | ❌ Pendiente |
| `FIND-32` | 🟡 | Tests unitarios rate-limit en `vantadb-server/tests/server.rs` obsoletos: `test_rate_limit_enforces_after_burst` (:223) y `test_rate_limit_health_unaffected` (:235) asumen burst=1 con rpm=5, pero `rate_limit_burst` (REST-01) devuelve `rpm` completo sin auth (burst=5) → 2ª request protegida pasa 200 en vez de 429. Fallan en base (confirmado con `git stash`), excluidos del profile default (binary(server)); solo e2e cubre rate-limit (12/12). Fix: alinear tests a burst=rpm (o usar auth para burst=rpm/10) | `vantadb-server/tests/server.rs:223,235` · `src/cli_server.rs:172` rate_limit_burst · hallazgo VERIFY MOD-13 2026-08-25 | ❌ Pendiente |
...
>
> **Adenda 2026-08-24 (smoke E2E + conflicto multi-instancia):**
...
> - **Conflicto multi-instancia**: una segunda instancia opencode trabajó en paralelo sobre el mismo working tree y **sobrescribió el lote UX-02..05 aplicado** (teclado grid, wiring de focus trap, labels IngestForm, token `label-tech`). Solo sobrevivió `desktop/src/components/ingest/useModalFocus.ts` (commiteado huérfano, sin wiring). Los UX-02..05 siguen pendientes — re-aplicar sobre el estado actual cuando no haya otra instancia activa.
> - **Regla de proceso**: 1 instancia opencode por working tree (o git worktrees separados). Dos agentes editando en paralelo se pisan checkpoints y revierten trabajo del otro.
> - La segunda instancia continúa su propia campaña (`docs/plans/2026-08-24-batch-review-mod-find.md`, MOD-02, FIND-27 llm.rs) — coordinar antes de tocar `desktop/src`.

| ID | Effort | Descripción | Archivos | Estado |
|----|--------|-------------|----------|--------|
| `UX-01` | 🟡 | **LensShell compartido**: 3 patrones de shell distintos entre lenses (Retrieval/Trash/Activity usan `section.press-lg.border-4` + header `border-b-4`; Indices/Consolidate divs sueltos; MemoryLens sin header) + 3 escalas de título (3xl/2xl/ninguno). Extraer `<LensShell title subtitle>` y adoptarlo en las 6 | `RetrievalLens.tsx:225`, `TrashLens.tsx:51`, `ActivityPanel.tsx:105`, `IndicesLens.tsx:93`, `ConsolidateLens.tsx:431`, `MemoryLens.tsx:409` | ✅ (cubierto por batch UX-01+05 — LensShell adoptado en las 6 lentes: Consolidate/Indices/Retrieval/Graph/Space/Memory; pendiente commit del lead) |
| `UX-02` | 🟡 | **Filas del grid no navegables por teclado**: `<tr onClick>` sin tabIndex/onKeyDown/aria-selected (igual en ResultsList `<li>`). Teclado/SR no puede abrir el Inspector desde el grid. *Lote aplicado 2026-08-24 y verificado en smoke (Enter→Inspector), sobrescrito por checkpoint concurrente — re-aplicar* | `DataExplorer.tsx:859-871`, `ResultsList.tsx:22-27` | ⬜ Pendiente |
...
| `UX-16` | 🔴 | **Dependencia fantasma `lucide-react`**: WorkspaceShell y DataExplorer importan `lucide-react` (Asterisk/Moon/Search/Settings/Sun/Trash2/TriangleAlert) pero NO está en `desktop/package.json` — resuelve solo por hoisting de node_modules. Romperá `npm ci` limpio, el build Tauri y cualquier CI. Agregar al package.json (o revertir a glifos texto como el resto del sistema) | `desktop/package.json`, `WorkspaceShell.tsx:19`, `DataExplorer.tsx` | ⬜ Pendiente |
...
| ID | Effort | Descripción | Archivos | Estado |
|----|--------|-------------|----------|--------|
| `AGT-01` | 🟢 | **Commit pendiente de fixes ya aplicados**: diff sin commit en `.opencode/AGENTS.md` (metadata manual) + borrado staged de `.opencode/instruccion-unificacion-skills.md` (instrucción de trabajo 2026-07-26 completada: 0 referencias entrantes verificadas por grep, entregable `unified-review/SKILL.md` existe). Al commitear, verificar que la sesión paralela no los haya restaurado | `.opencode/AGENTS.md`, `.opencode/instruccion-unificacion-skills.md` | ⬜ Pendiente |
| `AGT-02` | 🟢 | **Verificar stats CodeGraph hardcodeadas**: `.opencode/AGENTS.md` dice "7.3K símbolos, 24.7K edges" — contrastar contra `.codegraph/codegraph.db` (intento de verificación falló por quoting de shell, no por datos inválidos). Refrescar números o quitarlos si drift es inevitable | `.opencode/AGENTS.md` § CodeGraph | ⬜ Pendiente |
| `AGT-03` | 🟡 | **Spot-check refs `file:line` de tabla deuda P2 (Regla 6)**: el código evolucionó desde su registro — verificar vigencia de P2-1 (`vantadb-wasm/src/opfs.rs:83-87`), P2-3 (`convert.rs:23-70`), P2-5 (`lib.rs` ~312), P2-6 (`types.rs:365`), P2-7 (`serialization/mod.rs:227-294`), P2-8 (`wasm/lib.rs:402-433`); actualizar refs o migrar la tabla a issues con label `debt` para dejar de mantener líneas en AGENTS.md | `.opencode/AGENTS.md` Regla 6 | ⬜ Pendiente |
| `AGT-04` | 🟡 | **Limpieza `.opencode/opencode-loop/`**: 30+ archivos `ses_*.json.corrupt-*` y `ses_*.json.<pid>.<ts>.tmp` acumulados junto a cientos de sesiones vivas — borrar los corrupt/tmp y agregar rotación/gc al loop server para que no se acumulen de nuevo | `.opencode/opencode-loop/` | ⬜ Pendiente |
