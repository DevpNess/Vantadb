...
| FIND-25 | 🟡 | `create_snapshot` copia solo archivos top-level del data_dir (`read_dir` + `is_file()`) — subdirectorios (ej. `wal/`) NO se capturan; y no hay quiescing ni flush previo → snapshot durante writes activos puede ser torn. Gap de corrección independiente del restore. Origen: RES-02 (verificado mod.rs:521-527/:554-560) | src/storage/engine/mod.rs:507/:540 · research res02-backup-restore.md §1 | 🟢 | 🔴 Alta | Pendiente |
| FIND-26 | 🟢 | `WalArchiver`/`PitrRestorer` (src/wal_archiver.rs:56/:219, con restore_to_timestamp) son dead code — cero wiring desde el engine (rotación nunca llama archive_segment). Wire la rotación de segmentos o remover el módulo. PITR necesita base snapshot + replay (modelo base+log). Origen: RES-02 | src/wal_archiver.rs · research res02-backup-restore.md §2b | 🟠 | 🟡 Media | Pendiente |
