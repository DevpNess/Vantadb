...
| `UX-03` | 🟡 | **Focus trap en modales**: ImportPaste/ImportDrop tienen `role="dialog" aria-modal` pero Tab escapa al shell y no restauran foco al cerrar; ImportDrop sin autofocus | `ImportPaste.tsx:107-115`, `ImportDrop.tsx:124-133` | ⬜ Pendiente |
| `UX-04` | 🟡 | **Labels visibles en IngestForm**: solo-placeholder (aria-label no cuenta, WCAG 3.3.2); errores van a toast global sin anclaje inline; `window.confirm` nativo inconsistente con el patrón inline de dos pasos | `IngestForm.tsx:44-74` | ⬜ Pendiente |
| `UX-05` | 🟡 | **Token `.label-tech`**: `font-tech text-[10px] uppercase tracking-widest` repetido 15+ veces en el shell — extraer clase compartida en index.css | `WorkspaceShell.tsx` (~15 usos) | ⬜ Pendiente |
