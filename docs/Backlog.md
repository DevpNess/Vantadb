...
|----|-----|-------------------|------------|--------|
| `FIND-01` | 🔴 | Leak del modelo interno de metadata en TS: README y JSDoc exponen forma serde del enum Rust (`{ lang: { String: "en" } }`, `{ source: { type: "String", value } }`). Aceptar strings planos + normalizar internamente; actualizar ejemplos. Primera impresión del SDK | `vantadb-ts/README.md`, `vantadb.ts:408` · ts.md nit L50 (no trackeado antes) | ❌ Pendiente |
| `FIND-02` | 🔴 | Sin historia de embeddings para usuarios: quickstart usa vectores de juguete `[1,0,0]`; infraestructura existe (`VANTA_EMBEDDING_PROVIDER`, feature `remote-inference`) pero no aparece en QUICKSTART ni READMEs. Agregar sección Embeddings + evaluar helper dev-mode | `docs/QUICKSTART.md`, CONFIGURATION.md env vars | ❌ Pendiente |
| `FIND-03` | 🟡 | Doc drift superficie de entrada: PYTHON_SDK.md dice `import vantadb_py as vantadb` (QUICKSTART/README dicen canónico `import vantadb`); lista wheels linux/macOS pero OMITE Windows (release-wheels-60.yml sí lo builda); pyproject description "Source-installed..." es jerga interna visible en PyPI | `docs/api/PYTHON_SDK.md`, `vantadb-python/pyproject.toml` | ❌ Pendiente |
| `FIND-04` | 🟡 | Taxonomía cross-SDK no surfaceada al usuario: `search()` es ANN puro en Python e híbrido en TS (mismo nombre, semántica distinta); sufijos `_memory` solo en Python. Surfacear tabla de mapeo cross-SDK en ambos READMEs (link BINDINGS_NAMESPACES.md §hazard) | `docs/api/BINDINGS_NAMESPACES.md`, ambos READMEs | ❌ Pendiente |
| `FIND-05` | 🟢 | APIs fantasma documentadas: PYTHON_SDK.md documenta `delete_by_filter/similar_to_key/count` como "(not yet exposed)". Mover a sección Roadmap o quitar (parcialmente relacionado MOD-19) | `docs/api/PYTHON_SDK.md` | ❌ Pendiente |
| `FIND-06` | 🟢 | Consistencia idioma/frescura: READMEs SDK en español vs regla técnico=en inglés; `last_reviewed` julio sin gate de paridad docs↔código para PYTHON/TS_SDK.md (GOV-B4 solo cubre openapi/MCP) | READMEs, frontmatter docs/api | ❌ Pendiente |
| `FIND-07` | 🔴 | HTTP: `require_auth=false` default + host configurable a `0.0.0.0` SIN API key queda expuesto sin auth y sin bloqueo (solo log informativo). Decidir política: refuse-to-start o warning fuerte al exponer no-loopback sin key | `src/config.rs:649-659`, `cli_server.rs:1675-1690` · research journey 2026-08-23 | ❌ Pendiente |
| `FIND-08` | 🟡 | Ejemplos rotos/isla: `examples/demo/README.md` instala desde TestPyPI ≥0.4 (trampa primera-toma, contradice Colab); 10 ejemplos sin referenciar en ningún README (crewai/haystack/autogen/langgraph/langchain_ollama/agent_memory + 4 Rust) | `examples/demo/README.md:20,27-29`, `README.md:131-180` | ❌ Pendiente |
| `FIND-09` | 🟡 | Setup MCP indescubrible para usuarios externos: pasos por-IDE viven en `skills/vantadb-mcp/SKILL.md` interno; README raíz lo marca "Experimental/not MVP"; no hay quickstart MCP público. Decidir posicionamiento y crear guía pública | `README.md:221`, `docs/api/MCP.md:12` | ❌ Pendiente |
| `FIND-10` | 🟡 | TS packaging/errores: package ESM-only sin export CJS (`require("vantadb")` falla); errores del engine colapsan a `WASM_ERROR` genérico con string — usuario JS no distingue corrupto/not-found/validación desde código | `vantadb-ts/package.json`, `dist/errors.js`, `src/vantadb.ts` wrapWasmError | ❌ Pendiente |
...
| `FIND-12` | 🔴 | Sin docs-site alojado: `book/` mdbook compilado existe pero no hay hosting; `vantadb.dev` sin DNS (BND-07 cubre DNS externo, NO la decisión de hosting). Decidir: GitHub Pages con book existente (~1 día) vs sitio propio. Bloqueante de conversión para Show HN | `book/book/*.html`, README links | ❌ Pendiente |
| `FIND-13` | 🟡 | Política de estabilidad/deprecación no escrita para consumidores 0.x: qué se promete entre minors (breaking OK hasta 1.0 es estándar pero hay que declararlo). Sección corta en README + CONTRIBUTING; base para release notes de breaking changes (release-plz ya las detecta) | `README.md`, `CONTRIBUTING.md`, cliff.toml | ❌ Pendiente |
| `FIND-14` | 🟡 | Sin guía de upgrade entre versiones propias: existen migraciones DESDE SQLite/LanceDB pero no "0.x→0.y qué cambia para vos" por release. Definir formato (sección Upgrade Notes en release notes via release-plz) y escribir retroactivo 0.4→0.5 | `docs/operations/SQLITE_MIGRATION_GUIDE.md` (patrón), docs/CHANGELOG.md | ❌ Pendiente |
| `FIND-15` | 🟡 | Sin página comparativa honesta vs alternativas (sqlite-vec, LanceDB, Qdrant-embedded, Chroma) con números reproducibles según Regla 11 (bench file + comando + entorno). Primera pregunta del thread de HN: "¿por qué no X?". Incluye tabla de límites prácticos única (max dims, nodos recomendados, RAM por 1M vectores — hoy dispersa en PYTHON_SDK.md ID limits) | `docs/operations/BENCHMARKS.md`, `benches/canonical_p99.rs` · readiness eval 2026-08-23 | ❌ Pendiente |
| `FIND-16` | 🟡 | Backup/restore user-facing inexistente: runbook DR es interno (GOV-B2 lo validó); falta página simple para usuario final de DB embebida ("cómo respaldar tu data": flush → copy dir / export_all JSONL / restore) reusando procedimiento validado GOV-A3 | `docs/operations/DISASTER_RECOVERY_RUNBOOK.md` (interno) | ❌ Pendiente |
...
| `FIND-18` | 🟢 | NOTICE file ausente (recomendado por Apache-2.0 para atribución de terceros); cargo-deny lista deps pero el artefacto distribuido no lleva atribuciones consolidadas | raíz repo, deny.toml | ❌ Pendiente |
| `FIND-19` | 🔴 | Desktop sin title bar personalizada: controles de ventana fuera del diseño → sensación "web embebida". Investigar Tauri window customization (decorations:false + custom titlebar integrada al WorkspaceShell, inset controls por SO) | `desktop/src/components/layout/WorkspaceShell.tsx`, v2.tauri.app/learn/window-customization · research Studio 2026-08-23 | ❌ Pendiente |
...
| `FIND-22` | 🔴 | Tema via localStorage+class toggle en vez de seguir el OS: App.tsx lee localStorage y togglea .dark manualmente. ANALIZAR→INVESTIGAR→SOLUCIONAR→IMPLEMENTAR: preferencia sistema (prefers-color-scheme / tauri theme API), transición suave, respetar elección manual del usuario sobre la del OS. Incluye implementación completa, no solo análisis | `desktop/src/App.tsx:13-19` · research Studio 2026-08-23 | ❌ Pendiente |
| `FIND-23` | 🔴 | Sin splash/intro animada: arranque en frío muestra carga de sitio web. Intro con Remotion/Hyperframes usando logo + mascota Mark (ya portada a desktop con estados idle/loading/error en mark-studio.tsx — base perfecta). Definir duración ≤2s, skip-on-click, fallback estático | `desktop/src/components/mark/mark-studio.tsx`, skills remotion/hyperframes instaladas | ❌ Pendiente |
| `FIND-24` | 🔴 | Contraste sin calibrar entre temas: evidencia medida hoy — `--accent-foreground #FBF9F5` sobre `--accent #FF5500` ≈ 3.0:1 FALLA WCAG AA (texto normal); destructive == accent (mismo color para errores que highlights); dark usa borders crema full-contrast (`--border #FBF9F5` sobre #0a0a0a = harsh); web/ ni siquiera tiene bloque .dark. Auditoría WCAG por par foreground/background×superficie + recalibrar paleta manga/linocut con OKLCH (skill color-expert) manteniendo identidad | `desktop/src/index.css:73-160`, `web/src/app/globals.css:58-74` · medición 2026-08-23 | ❌ Pendiente |
| `FIND-25` | 🟡 | Home page y estructura de lenses sin lenguaje de usuario: existe HomeOverview pero falta evaluarlo como home real (estado DB, KPIs, accesos rápidos); nombres técnicos de lenses (Retrieval/Consolidate/Indices) → renombrar a lenguaje usuario (Memoria/Búsqueda/Grafo/Espacio) + guía de uso integrada first-run + help panel | `desktop/src/components/home/HomeOverview.tsx`, `layout/WorkspaceShell.tsx:429` | ❌ Pendiente |
| `FIND-26` | 🟡 | Design system duplicado sin gobernanza: desktop clona literalmente los tokens de web (`#FBF9F5/#000000/#FF5500` + mismas 4 tipografías) sin mecanismo de sync — drift garantizado. DECIDIR arquitectura: paquete compartido tokens (npm workspace) vs desacople intencional (identidad app ≠ identidad marketing). Incluye decisión: ¿web necesita dark theme? (hoy light-only, sin bloque .dark en globals.css) | `desktop/src/index.css:73-160`, `web/src/app/globals.css:58-74` · research Studio 2026-08-23 | ❌ Pendiente |
| `FIND-27` | 🟢 | Mismo patrón frágil que REVIEW-14 arregló en explain.rs: unwraps estructurales de `query_sparse.as_ref().unwrap()` tras bool-guard en paths de búsqueda — bindear `Some(qs)` en el pattern del match (fix mecánico, mismo shape que REVIEW-14; tocar search hot path → verificar suite completa) | `src/sdk/search/mod.rs:207,240,265,315,346,369`, `src/sdk/search/debug_ops.rs:288,335,374` · hallazgo DISCOVERY REVIEW-14 2026-08-23 | ❌ Pendiente |
