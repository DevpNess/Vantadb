...
| FIND-62 | Alta | **`commit_transaction` corre WAL batch + apply SIN `insert_lock` (origen: spike FIND-61 2026-09-04, viola ERR-010):** `txn.rs:119-213` hace `batch_append` (L160) + apply loop (L167-195, solo `try_push` oportunista sin `drain_hnsw_batch_locked`) sin `acquire_insert_lock` (`rg insert_lock txn.rs` = 0 verificado). Un `flush()` concurrente puede drenar-vacío → serializar → contar (incluye esos records) → checkpoint → el commit pushea tarde = record invisible en recovery. `apply_insert_with_txn`/`stamp_deleted_in_backend` tampoco toman el lock. Fix: envolver el apply del commit en `insert_lock`. Contrato: test de interleaving flush-vs-commit verde + suite storage sin regresión | `src/storage/engine/txn.rs:119-213`, `src/storage/engine/maintenance.rs:47-93` | 🟡 1-2d | 🔴 Alta (correctness/durabilidad) | ⬜ Pendiente |
| FIND-63 | Baja | **Rama muerta `SyncMode::Never` en `maybe_sync` (origen: spike FIND-61 2026-09-04):** `wal.rs:376-389` solo ramifica `Always` vs else-threshold; `rg Never src/` = definición+parsing, 0 branches — con threshold None, `Never` fsyncea igual que Periodic. Cualquier bench futuro que use `Never` ingenuamente mide ruido (el spike lo neutralizó con `Never*` bench-only). Fix: ramificar `Never` explícito O documentar que no existe | `src/wal.rs:376-389`, `src/config.rs:102/1308` | 🟢 <1h | 🟢 Baja | ⬜ Pendiente |
