...
## Phase 11: 🧠 Embeddings Local-First — `embeddings/` + 9 modelos + `embed-local` (2026-08-28) — ✅ COMPLETADA 2026-08-28
...
> **Plan:** `docs/plans/2026-08-28-embeddings-local.md` (9 tareas EMB-01..09) — **9/9 ✅ COMPLETADAS 2026-08-28** (commits `2c185021`→`d24eeb1c`, ver `git log --oneline --grep=EMB-`). Campaña `run docs/plans/2026-08-28-embeddings-local.md` (FAIL_MODE=parallel: Wave0 EMB-01, Wave1 EMB-02, Wave1b EMB-03, Wave2 EMB-04/05/06 paralelo, Wave3 EMB-07, Wave4 EMB-08, Wave5 EMB-09).
> **Carpeta:** `embeddings/` creada en EMB-01 con `manifest.json` (9 modelos, rev pinned), `download.py` (`--only/--skip-exception/--check/--dry-run`), `verify.py`, `README.md`, `manifest.lock`, `models/` gitignored (`/embeddings/models/`).
> **Modelos:** 8 ≤3GB + 1 excepción `Qwen3-Embedding-8B` (16GB, MTEB 75.1). Todos ONNX+HF salvo Qwen3 (HF only). Balance 3 EN / 3 ES / 3 Combined.
> **Fix puntos 1,3,4:** Punto1 (hoy BYO-vector) → `embed-local` feature `Cargo.toml:97` + `LocalOnnxProvider` `src/llm.rs:132` + docs `QUICKSTART/EMBEDDINGS`; Punto3 (qué módulo necesita) → `vanta-memory` L1 (`EMB-04`), MCP `embed_texts` (`EMB-05`), SQL auto-embed (`EMB-06`); Punto4 (plan 4 fases) → ejecutado.

| ID | Descripción (→ Resultado) | Archivos | Esfuerzo | Prio | Estado |
|----|-------------|----------|----------|------|--------|
| `EMB-01` | **Infra `embeddings/` + manifest + `download.py` + `verify.py` + `.gitignore`** — 5 archivos + `manifest.lock` + delta `.gitignore` + `README` 9 modelos. | `embeddings/**`, `.gitignore:72` | 🟢 4-6h | 🔴 Alta | ✅ `2c185021` |
| `EMB-02` | **Feature `embed-local` + `LocalOnnxProvider` (ort+tokenizers)** — `Cargo.toml:97` + `src/llm.rs:132` + `src/config.rs` `VANTA_LOCAL_MODEL`. | `Cargo.toml:97`, `src/llm.rs:26,39,132`, `src/config.rs` | 🟠 3-5d | 🔴 Alta | ✅ `9e06e79e` |
| `EMB-03` | **Descarga + verificación 9 modelos** — `download.py --check` 9 modelos + `verify.py --check` + `--dry-run` + `verify.log` tabla 9 PASS check-only (smoke 691MB deferido rev 404). | `embeddings/models/**`, `embeddings/manifest.lock`, `embeddings/verify.log` | 🟡 1d wall | 🔴 Alta | ✅ `f5cb880c` |
| `EMB-04` | **Cablear `vanta-memory` L1 (fix punto 3)** — `vanta-memory` hook `L1DedupConfig::with_local_provider` + `local_embedding_hook` 384d dummy + `auto_recall.rs:69` doc. | `vanta-memory/Cargo.toml:40`, `vanta-memory/src/core/record/*`, `vanta-memory/src/core/hooks/auto_recall.rs:69` | 🟡 1-2d | 🔴 Alta | ✅ `3075df46` |
| `EMB-05` | **MCP tool `embed_texts` (fix punto 3)** — `vantadb-mcp/src/lib.rs` + `handlers/tools.rs:549` `embed_texts` 1-128 texts, budgeting 25k, `embed_batch` fallback dummy. | `vantadb-mcp/src/handlers/tools.rs`, `vantadb-mcp/src/config.rs`, `src/llm.rs:26` | 🟢 4-6h | 🟡 Media | ✅ `241b0868` |
| `EMB-06` | **SQL vector auto-embed (fix punto 3)** — `src/physical_plan.rs:51,129` `#[cfg(embed-local)]` branches con `LocalOnnxProvider`. | `src/physical_plan.rs:51,129` | 🟢 2-4h | 🟡 Media | ✅ `bd9ab5ca` |
| `EMB-07` | **Bench comparativo 9 modelos** — `benchmarks/embed_bench.py` 533L + `docs/operations/BENCHMARKS.md` §8 + report 9 filas. | `benchmarks/embed_bench.py`, `docs/operations/BENCHMARKS.md` | 🟡 1d | 🟡 Media | ✅ `67fda296` |
| `EMB-08` | **Docs + Quickstart multi (fix punto 1)** — `docs/QUICKSTART.md:182` + `docs/tutorials/05-embedding-integrations.md` + `docs/api/EMBEDDINGS.md` + `EXPERIMENTAL_FEATURES.md:60` + `README.md:232`. | `docs/QUICKSTART.md:182`, `docs/tutorials/05-embedding-integrations.md`, `docs/api/EMBEDDINGS.md` | 🟢 4h | 🟡 Media | ✅ `86cdfc57` |
| `EMB-09` | **Excepción Qwen3 >3GB — wiring + doc** — `embeddings/README.md` sección + manifest `onnx:null` + bench `--include-exception`. | `embeddings/README.md`, `benchmarks/embed_bench.py` | 🟢 2h | 🟢 Baja | ✅ `d24eeb1c` |
...
> **Registro avance:** `docs/progreso/README.md` § EMB (9/9) + `docs/plans/archive/2026-08-28-embeddings-local.md` (al cerrar campaña).
...
| AUD-043 | Alta | Fix clippy `unused variable: ns` en closure `options_for` (`src/cli_server.rs:1302`, commit `abc4ec10`) que rompe `just verify` / pre-push gate / CI Fast Gate. Fix: renombrar parámetro a `_ns` (~2 min). Origin: audit-full-20260825-010607 | `src/cli_server.rs:1302` | 🟢 | 🔴 Alta | ✅ Completada (2026-08-31, fast-gate-residues Task 1) — target arqueológico: tras REVIEW-10 (`cf2ecc50`) el closure vive en `src/server/routing.rs:1166` con `_ns: String` aplicado. Cero código editado. Lint cascade residual (unused imports + assertions_on_constants) registrado como FIND-035 (Task 1 del plan arqueológico, ver `.opencode/skills/campaign-executor/tasks/AUD-043.md`). |
...
| FIND-35 | Alta | Lint cascade residual rompe `cargo clippy -p vantadb --all-targets --all-features -- -D warnings` (rompe fast gate post-AUD-043): 5 unused imports en `src/server/routing.rs:11,12,13,17,42,57` (`simple_url_decode`, `ConversationTrigger`, `REQUEST_ID_HEADERS`, `REQUEST_ID_MAX_LEN`, `AuditLogger`, `parking_lot::Mutex`, `RbacConfig`) introducidos por REVIEW-10 split + 1 `clippy::assertions_on_constants` en `src/config.rs:1767` (`assert!(MAX_K >= 1_000, ...)` con `MAX_K` const) + 3 errores derivados en lib-test. Origen: derivado durante Task 1 (AUD-043 arqueológico) del plan 2026-08-31-fast-gate-residues. Acción: drop imports + wrap assert en `const { assert!(...) }` (Rust 1.79+) o `#[allow(clippy::assertions_on_constants)]` con justificación. Verificar `cargo check -p vantadb` + `cargo clippy -p vantadb --all-targets --all-features -- -D warnings` exit 0. NO scope-creep a AUD-043 (mantener paridad). | `src/server/routing.rs:11,12,13,17,42,57`, `src/config.rs:1767` | 🟢 | 🔴 Alta | Pendiente (derivada 2026-08-31) |
| FIND-36 | Media | `cargo check -p vantadb-mcp --tests` falla en `test_embed_texts.rs:78` — campo `max_embed_batch_size` no existe en `McpConfig` (campos disponibles: `max_concurrency`, `max_payload_length`, `max_key_length`, `max_namespace_length`, `max_vector_dim`, etc.). Origen: detectado durante Task 2 (FIND-MCP-001 arqueológico) del plan 2026-08-31-fast-gate-residues. Acción: agregar `max_embed_batch_size` a `McpConfig` O corregir test para usar campo existente. Effort 🟢 ~10 min. | `vantadb-mcp/tests/test_embed_texts.rs:78`, `vantadb-mcp/src/config.rs` | 🟢 | 🟡 Media | Pendiente (derivada 2026-08-31) |
