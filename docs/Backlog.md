...
| FIND-34 | Alta | Ciclo WAL Writer (4 nodos): `open`↔`open_with_buffer`↔`recover_valid_records`↔`quarantine_corrupt_tail` — recursión en writer crítico sin tests de coverage en recovery/quarantine. Origen: codegraph-20260827-143245 Fase 1 | src/wal.rs:202-575 | 🟠 | 🔴 Alta | Pendiente |
| FIND-35 | Alta | Ciclo StorageEngine get/prefetch (2 nodos): `StorageEngine.get`↔`prefetch_related` — ciclo en read path sin justificación documentada. Origen: codegraph-20260827-143245 Fase 1 | src/storage/engine/get.rs | 🟠 | 🔴 Alta | Pendiente |
| FIND-36 | Alta | Cross-crate NativeConnection ↔ RocksDbBackend (3 ciclos get/put/delete): Tauri frontend llama a backend y backend llama a frontend — violación de frontera crate. Origen: codegraph-20260827-143245 Fase 1 | desktop/src-tauri/src/connections/native.rs ↔ src/backends/rocksdb_backend.rs | 🟠 | 🔴 Alta | Pendiente |
...
| FIND-39 | Media | `ScalarIndex.remove` sin test: método público con score semántico 0.98 vs tests pero sin test dedicado en `src/storage/engine/tests/`. Origen: codegraph-20260827-143245 Fase 10 | src/scalar_index.rs | 🟢 | 🟡 Media | Pendiente |
...
| `MCP-36` | **🔴 P0 Protocolo moderno: negociación protocolVersion + structured tool output** — `initialize.rs:11` hardcodea `"protocolVersion": "2024-11-05"`; spec estable actual es 2025-06-18 (structured tool output, annotations, elicitation) y latest 2026-07-28. Acciones: (1) negociar eco de versión del cliente con anuncio 2025-06-18; (2) evaluar structured tool output para tools clave; (3) NO implementar Roots/Sampling/Logging (deprecados 2026-07-28). Fuente: modelcontextprotocol.io/specification/2026-07-28 changelog. Ref: docs/reviews/mcp-research-20260825.md §3 | `vantadb-mcp/src/handlers/initialize.rs` | 🟢 | 🔴 | ❌ Pendiente |
...
| `MCP-38` | **🔴 P0 Tool annotations** — 0 annotations en toda la superficie (grep vacío); directorios oficiales (ChatGPT plugins, Claude Connectors) empiezan a exigirlas. Añadir `readOnlyHint`/`destructiveHint`/`idempotentHint`/`openWorldHint` a las 75 tools según semántica real (mayoría CRUD de lectura trivial). Fuente: blog.modelcontextprotocol.io/posts/2026-03-16-tool-annotations. Ref: docs/reviews/mcp-research-20260825.md §6 P0-C | `vantadb-mcp/src/*.rs` (definiciones de tools) | 🟢 | 🔴 | ❌ Pendiente |
...
| `TS-05` | **Preservar `engines` en el tarball publicado**: hoy repo declara `node>=22.12` pero registry tiene `engines:null` → `require(esm)` falla confuso en Node<22 sin aviso previo (H-04). Verificar causa (release-plz/npm config) + test del publish | `vantadb-ts/package.json:6-8`, pipeline npm (`release-npm-61.yml`) | 🟢 | 🟡 Media | ⬜ Pendiente |
...
| ID | Descripción | Archivos clave | Esfuerzo | Prioridad | Estado |
|---|---|---|---|---|---|
| `WSM-01` | **Eliminar fallback silencioso OPFS→in-memory**: `OpfsStorage::open(path).await.ok()` traga el fallo y `save()` queda no-op sin señal. Propagar error O caer a IndexedDB con warning explícito; `capabilities().persistence` debe reflejar el backend real post-connect. Test: simular fallo getDirectory y verificar error/fallback+warning | `vantadb-wasm/src/lib.rs:425`, `opfs.rs`, `idb.rs` | 🟡 | 🟠 Alta | ⬜ Pendiente |
...
| `WEB-07` | **Política sandbox playground**: `new Function` self-XSS documentado (`code-playground.tsx:318-323`) — sandbox iframe antes de cualquier exposición pública del playground a terceros; doc de la decisión. Origen: INV-web-01 H-04 | `web/src/components/vanta/code-playground.tsx:318-323` | 🟡 | 🟡 Media | ⬜ Pendiente |
...
|---|---|---|---|---|---|
| `STABLE-00` | **Checklist y ADR de promoción** — Escribir `docs/architecture/adr/ADR-XXX-default-members-promotion.md` con criterios arriba + tabla de coste por crate (tiempo `cargo check`/`clippy`/`nextest`, toolchain extra, tamaño `Cargo.lock`). Gate: `question` al owner para aprobar el umbral Fast Gate (<5 min vs Heavy) antes de tocar `Cargo.toml`. | `Cargo.toml:636`, `docs/operations/CI_POLICY.md`, `docs/architecture/adr/`, `dev-tools/verify.ps1` | 🟢 4h | 🔴 Alta | ⬜ Pendiente |
