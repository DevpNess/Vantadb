...
> **Total open items:** ~40 activas (conteo real 2026-08-22, 40 filas `❌ Pendiente`; regla GOV-C7: actualizar contando filas pendientes en cada sync; MCP-20/26 cerradas en este sync) — previas (DISC-01..03, LEG-01, BIZ-01b, OLD-01, DESKTOP-15..27, [ADMIN-XX pending W4]) + P15 residuales (ERR-006/007/008/009/015/026/031/032/033/036/037/042/043/044/045/047/048/049) + P16 residuales (PERF-07/08/09, CI-01) + **P25 (MCP-16..26) + P26 (VS-00..11 + VS-CORE-01..07)**. P15/P16 principales ejecutadas por plan `docs/plans/archive/2026-08-09-backlog-pipeline.md` (49/49 ✅). P19 CI batch (CI-02..07) ejecutado y migrado 2026-08-12 — plan archivado `docs/plans/archive/2026-08-12-ci-deuda.md`. Origen: investigación multi-agente 2026-08-09 → `docs/research/investigacion-equipo-2026-08-09.md`
...
| **P12** 🖥️ DESKTOP App (Tauri) + Consola Admin | 17 pendientes (DESKTOP-23..27 re-scopeadas + **28..39 nuevas 2026-08-22**: unificación estética, polling, settings, CRUD ns, CONSOLIDAR completo, lente MEMORIA para vanta-memory, dashboard PROXY; ADMIN-01..09 + DESKTOP-20 ✅; 10 archivadas por P26) | 📆 Backlog | 🟡 Media |
...
| **P32** 🔬 Reviews de Módulos (14 reportes `docs/reviews/modulos/`) | 62 (MOD-01..62: 14 🔴 / 34 🟡 / 14 🟢) — top: WAL resurrect core, MCP notifications, ensure HTTP, pytest suite rota, node superficie 11/42 | 📆 Backlog | 🟠 Alta (bloqueantes sistémicos identificados) |
...
| `DESKTOP-28` | **Unificar paneles legacy al design system Studio + limpieza** — Migrar `ConnectionPanel/MetricsGrid/KpiCards/SopPanel/ExportPanel/IngestForm/DataExplorer` de `.panel`/App.css a Tailwind manga/linocut (tokens `index.css`). Eliminar huérfanos nunca montados: `components/SearchBar.tsx`, `components/ProcessPanel.tsx`. Decisión SopPanel: botones WAL Replay/Reindex son falsos (solo leen métricas, triggers no existen en core — verificado ADMIN-06) → quitar botones o etiquetar "solo lectura". **DoD:** RESUMEN sin estética mixta; 0 componentes huérfanos; SopPanel sin acciones muertas. | `desktop/src/components/*.tsx`, `desktop/src/App.css` | 🟡 | 🟡 | ❌ Pendiente |
| `DESKTOP-29` | **Coordinar polling de métricas** — Hoy 3 timers independientes piden lo mismo: MetricsGrid (4s) + KpiCards (5s) + IndicesLens (4s) + ExportPanel oneshot. Extraer 1 hook único (`useMetricsPoll`) con 1 suscripción compartida y consumers. **DoD:** 1 solo poller activo para `vanta_metrics`; deltas/trend intactos. | `desktop/src/hooks/useMetrics.ts`, `MetricsGrid.tsx`, `KpiCards.tsx`, `IndicesLens.tsx` | 🟢 | 🟡 | ❌ Pendiente |
| `DESKTOP-30` | **Estado de usuario durable** — Favoritos, historial de búsquedas, undo/papelera y preferencias de layout hoy mueren al cerrar (memoria/localStorage de sesión). Persistir en `app_config_dir` vía la infra de DESKTOP-23 (save atómico temp+rename). Complementa DESKTOP-23 (que cubre tema/layout/filtros); esta cubre favoritos★/undo/historial/papelera. **DoD:** reiniciar conserva favoritos, historial y papelera. | `desktop/src/store/*.ts`, `src/config.rs` | 🟢 | 🔵 | ❌ Pendiente |
| `DESKTOP-31` | **Pantalla SETTINGS** — No existe (tema es lo único configurable). Crear superficie SETTINGS: conexiones guardadas multi-perfil (path nativo / URL server), auth Bearer token para server remoto (hoy imposible conectar a server con auth), defaults de búsqueda (top_k, modo), idioma. Alimenta a `ConnectionPanel` y topbar. **DoD:** conectar a `vanta-cli server` remoto con token desde UI; perfiles persistidos y seleccionables. | `desktop/src/pages/Settings.tsx` (nuevo), `transport.ts`, `src/config.rs` | 🟡 | 🟠 | ❌ Pendiente |
| `DESKTOP-32` | **CRUD de namespaces** — Sidebar lista namespaces pero no permite crear/renombrar/borrar (namespaces se crean implícitos por primer put). Acciones: crear vacío, renombrar (copy+delete batch vía `delete_by_filter`), borrar con confirmación 2 pasos + papelera. **DoD:** crear/renombrar/borrar ns desde sidebar con feedback y undo donde aplique. | `WorkspaceShell.tsx`, `desktop/src/vanta.ts` | 🟢 | 🔵 | ❌ Pendiente |
| `DESKTOP-33` | **CONSOLIDAR: merge + delete reales** — ConsolidateLens detecta duplicados (kNN textual) y marca `metadata.superseded_by` vía put, pero no ofrece merge de payloads ni borrado del duplicado. Agregar: merge manual campo-a-campo (pre-fill desde registro dominante), delete del supersedido con confirmación, acción batch sobre selección. **DoD:** flujo detectar→revisar→merge/delete completo sin salir de la lente. | `ConsolidateLens.tsx`, `consolidate-core.ts` | 🟡 | 🔵 | ❌ Pendiente |
| `DESKTOP-34` | **Polish UX global** — (a) CommandPalette sin entradas para PAPELERA/RETRIEVAL/CONSOLIDAR/ESPACIO — agregar todas las superficies; (b) hints F1/F2 del sidebar sin tooltip de qué hacen; (c) UI mezcla ES/EN hardcoded (aria-labels EN, labels mixtos) — unificar a ES (idioma default del proyecto) sin framework i18n (YAGNI). **DoD:** Ctrl+K alcanza las 9 superficies; todo label visible en ES consistente. | `CommandPalette.tsx`, `WorkspaceShell.tsx`, componentes varios | 🟢 | 🔵 | ❌ Pendiente |
| `DESKTOP-35` | **Slider híbrido cableado a search_profile real** — RetrievalLens re-rankea BM25⟷vector client-side porque el core tenía RRF fijo. MEM-01/MEM-02 (search profile mode/rrf_k/candidate_k por request) ya implementados en core+MCP (campaña P27) — pasar el peso del slider como `search_profile` en el request y eliminar el re-rank client-side. **DoD:** resultados del slider == explain del server (RRF real, no aproximación local). | `RetrievalLens.tsx`, `retrieval-core.ts`, `desktop/src/vanta.ts` | 🟢 | 🔵 | ❌ Pendiente |
| `DESKTOP-36` | **Bridge Tauri vanta-memory** — Exponer comandos Tauri sobre los handlers tipados existentes (`vanta-memory/src/gateway/knowledge_handlers.rs`: scene_read/list/query) + lectura de persona snapshots (`persona/<session>`), skills versionadas (list/versions/restore read-only v1), generation-log L1/L2/L3 y compaction reports. Sin writes en v1 (observabilidad primero). Nota: vanta-memory NO tiene transporte MCP/REST aún — el bridge Tauri usa la crate directo (patrón core inlined `../..`). **DoD:** `vanta.ts` expone scene_list/persona_get/skill_list/genlog_query probados contra seed de `vanta-seed`. | `desktop/src-tauri/src/commands/memory.rs` (nuevo), `vanta-memory/src/gateway/`, `desktop/src/vanta.ts` | 🟡 | 🟠 | ❌ Pendiente |
| `DESKTOP-37` | **Lente MEMORIA (UI)** — Sexta superficie del Studio que visualiza el diferenciador del producto (hoy invisible): navegador de escenas ordenado por heat (soft-delete visible), snapshot de persona con diff entre versiones, skills versionadas (timeline por content-hash), timeline del generation-log filtrable por capa L1/L2/L3. Depende de DESKTOP-36. **DoD:** ver escenas/persona/skills/log de una sesión sembrada con `vanta-seed`; click → Inspector cuando el dato sea un record. | `desktop/src/components/memory/*` (nuevo) | 🔴 | 🟠 | ❌ Pendiente |
| `DESKTOP-38` | **Dashboard PROXY** — Visualizar `vanta-proxy` (TurnReports: protocolo/modelo/status/duración; sesiones activas team→agent→task con TTL; cola write-back pendiente; rate-limit hits). ⚠️ Pre-requisito: vanta-proxy solo expone `/health` hoy — requiere endpoint metrics/snapshot server-side primero (tarea previa en crate `vanta-proxy`, coordinar con owner). La UI consume REST del proxy (no bridge nativo — el proxy corre como proceso aparte). **DoD:** panel operativo con datos vivos de una sesión proxy real. | `desktop/src/components/proxy/*` (nuevo), `vanta-proxy/src/server.rs` | 🟡 | 🔵 | ❌ Pendiente |
| `DESKTOP-39` | **Ingest con embedding desde texto (condicional)** — IngestForm exige vector provisto; no hay forma de generar embedding desde texto en la UI. Verificar primero qué expone el core (`src/llm.rs` embeddings — confirmar API pública y modelo requerido); si existe path sin LLM externo, añadir botón "generar vector" en IngestForm/ImportDrop; si requiere modelo externo, documentar límite y cerrar como WONTFIT-UI. **DoD:** decidir y ejecutar según verificación (no inventar claim de embedding local). | `IngestForm.tsx`, `src/llm.rs` | 🟢 | 🔵 | ❌ Pendiente |
...
| REVIEW-13 | Baja | `supersede()` TOCTOU concurrente: guard lee `superseded_by` (:857) y marca en :886 sin atomicidad — dos threads pueden double-mark/overwrite divergente. Serializar vía write lock del engine o re-check en sección crítica WAL | src/sdk/api.rs:857-886 | 🟡 Abierta — review-full-20260822 H09-CODE-004 |
| REVIEW-14 | Baja | `version_history.rs:283` `k[k.len()-8..].try_into().unwrap()` panics con key <8 bytes (store corrupto) → devolver `VantaError::Corrupt`. Runner-up: unwraps frágiles `explain.rs:103/147/183` — bindear el Some en el pattern del match | src/sdk/version_history.rs:283, src/sdk/search/explain.rs | 🟡 Abierta — review-full-20260822 H09-CODE-005 |
| REVIEW-15 | Baja | Cast `from_raw_parts` a f32 sin assert de alineación (`vector_data.rs:167`) — invariante page-aligned implícito. Añadir `debug_assert!(ptr % 4 == 0 && len % 4 == 0)` o usar `align_to()` | src/node/vector_data.rs:167 | 🟢 Abierta — review-full-20260822 H07-UNSAFE-002 |
| REVIEW-16 | Baja | 5 imports muertos en `debug_ops.rs` (warnings en cada build maturin/cargo). `cargo fix --lib -p vantadb` y listo | src/sdk/search/debug_ops.rs:2 | 🟢 Abierta — review-full-20260822 L2-CODE-001 |
| REVIEW-17 | Baja | 7 bloques `unsafe` innecesarios bajo target wasm32 (vfile.rs:202,206,277; archive.rs:74,105; vfile_mmap.rs:73,112) — mmap safe en ese backend, markers stale. Remover o documentar por-plataforma (owner: vanta-engine) | src/storage/vfile*.rs, src/storage/archive.rs | 🟢 Abierta — review-full-20260822 L2-CODE-002 |
| REVIEW-18 | Baja | Warning Next.js: package-lock stray fuera del repo — fijar `turbopack: { root: __dirname }` en next.config.ts | web/next.config.ts | 🟢 Abierta — review-full-20260822 H03-CODE-001 |
| REVIEW-19 | Baja | No hay CHANGELOG.md en la raíz (canónico en docs/) — tooling que espera changelog root no lo encuentra. Stub de una línea apuntando a docs/CHANGELOG.md | CHANGELOG.md (nuevo) | 🟢 Abierta — review-full-20260822 H05-MISSING-001 |
| REVIEW-20 | Baja | Script validate-docs-coverage documentado NO existe; links de docs solo verificados ad-hoc. Crear script o paso CI que valide links internos automáticamente | dev-tools/scripts/ | 🟢 Abierta — review-full-20260822 H05-MISSING-002 |
...
| ID | Descripción (Gap → Acciones → Resultado) | Archivos | Esfuerzo | Prio | Estado |
|----|-------------|----------|----------|------|--------|
| `MCP-30` | **G15 🔴 Scenes API vía MCP** — Problema: el índice de escenas (navegación semántica de sesiones) solo es accesible desde desktop/proxy vía `scene_read`/`scene_list`/`scene_query` (`vanta-memory/src/gateway/knowledge_handlers.rs`) — un agente MCP no puede navegar su memoria estructurada por escenas. Acciones: (1) tools `scene_read`/`scene_list`/`scene_query` (wrappers directos de los handlers existentes — session_key + top_k); (2) schema JSON-RPC + patrón MEM-32; (3) tests round-trip con seed de sesión; (4) SKILL.md ×2. Resultado: navegación por escenas desde el agente. | `vantadb-mcp/src/handlers/tools.rs`, `vanta-memory/src/gateway/knowledge_handlers.rs` | 🟢 | 🔴 | ❌ Pendiente |
| `MCP-31` | **G16 🔴 Context engine vía MCP (assemble + recall)** — Problema: `assemble_with_recall` / compresión MMD (`vanta-memory/src/context_engine/engine.rs`, `compressor.rs`) — el ensamblado de contexto con presupuesto de tokens y recall — solo alimenta al desktop vía IPC (MEM-58 `vanta_context_assemble`). Es LA funcionalidad distintiva del "memory OS" y un agente externo no puede usarla. Acciones: (1) tool `context_assemble` (session_key, token_budget, query opcional) wrapper del engine; (2) decidir exposición del compresor (probablemente interna, solo assemble); (3) tests con sesión seedada; (4) docs. Resultado: agentes externos ensamblan contexto con la misma calidad que el desktop. | ídem + `vanta-memory/src/context_engine/` | 🟠 | 🔴 | ❌ Pendiente |
...
| `MCP-33` | **G18 🟡 Axiomas: write/update/delete vía MCP** — Problema: solo existe `read_axioms` (Iron Axioms read-only, `vantadb-mcp/src/axioms.rs` es resolve/read) — un agente no puede registrar ni actualizar sus propios axiomas (reglas invariantes persistentes). Acciones: (1) investigar si existe API de escritura en core (`src/agentic/axioms*`); si no, definirla como records en namespace reservado `_axioms` con convención documentada (evita cambiar core); (2) tools `write_axiom`/`delete_axiom`; (3) tests; (4) SKILL.md. Resultado: axiomas gestionables por el agente sin romper los Iron Axioms. | `vantadb-mcp/src/axioms.rs`, `vantadb-mcp/src/handlers/tools.rs` | 🟢 | 🟡 | ❌ Pendiente |
...
| `MOD-12` | server | 🔴 | `ensure_indexes_current` NO corre en path HTTP (solo MCP): búsqueda textual/híbrida rota en DB fresca vía HTTP. Fix 1-3 líneas + e2e text-search | `cli_server.rs:1758`, `builder.rs:35-42` · server.md 🔴 | ❌ Pendiente |
...
| `MOD-16` | python | 🔴 | Suite default rota: `pytest -q` = 66 failed (DBs sin cerrar acumulan RSS → guard memoria). Fixture autouse cierre DBs. Claim "70 passed" era parcial | `tests/test_async_smoke.py:35` · python.md H1 | ❌ Pendiente |
| `MOD-17` | python | 🔴 | Deadlock potencial: `OpGate::drain()` espera condvar sosteniendo GIL en close(). Mover drain dentro de py.detach + test estrés concurrente | `lib.rs:1711-1717,132-139` · python.md H2 | ❌ Pendiente |
| `MOD-18` | python | 🟡 | Stubs `.pyi` duplicados y desactualizados (put_batch type, métodos faltantes). Consolidar en 1 stub + test anti-drift firmas↔stub | `vantadb_py/__init__.pyi:56-302` · python.md | ❌ Pendiente |
| `MOD-19` | python | 🟡 | ~30% API core sin exponer: similar_to_key, count, delete_by_filter, versions, remove_edge (top-5 barato; mem0 vive de fallbacks) | `api.rs:1218,1343,1412,1520` · python.md | ❌ Pendiente |
| `MOD-20` | python | 🟡 | Excepciones genéricas catch-all RuntimeError + query() retorna string formateado. Jerarquía VantaError propia + retornar estructura | `convert.rs:659-684`, `lib.rs:1578` · python.md | ❌ Pendiente |
| `MOD-21` | python | 🟢 | Nits agrupados: artefactos stale commiteados (wheels/.pyd/.pdb), async graph_bfs pierde direction, validación inconsistente, MAX_K clamp silencioso, connect sin read_only/backend | `dist/`, `convert.rs:36-147` · python.md | ❌ Pendiente |
| `MOD-22` | ts | 🔴 | Tipos de grafo ficticios: GraphBfs/Dfs/TopoSort declaran {visited,levels,path}/{sorted,has_cycle} pero el wire real es `Vec<u128>` plano — blind-cast `as`, consumidor recibe undefined. Test debe afirmar shape real | `vantadb.ts:1057,1081,1103` vs `sdk/graph.rs:50` · ts.md | ❌ Pendiente |
| `MOD-23` | ts | 🔴 | `NativeVantaDB._native` nunca envuelve rechazos async (catch fuera del promise chain). Fix 3 líneas: async/await + wrapNativeError | `native.ts:88-94` · ts.md | ❌ Pendiente |
| `MOD-24` | ts | 🟢 | Nits agrupados: _mapRecord/_buildSearchRequest duplicados ts↔native, validateVector type-lie, ejemplos JSDoc no compilables, ESM-only sin documentar, coverage/ commiteada | `guards.ts:82`, `vantadb.ts:408,511` · ts.md | ❌ Pendiente |
| `MOD-25` | wasm | 🟡 | Fallback silencioso a in-memory si OPFS falla (`.ok()`): save() se vuelve no-op invisible para el usuario. Propagar error o caer a IDB con warning + capabilities().persistence honesto | `vantadb-wasm/src/lib.rs:425` · wasm.md | ❌ Pendiente |
| `MOD-26` | wasm | 🟡 | `OpfsFile::append` sobreescribe en offset 0 (la versión JS sí calcula posición) — API pública rota; arreglar o eliminar hasta necesitarlo | `opfs.rs:85-98` vs `opfs_bridge.js:53-57` · wasm.md | ❌ Pendiente |
| `MOD-27` | wasm | 🟡 | Cuotas storage sin manejar: sin estimate()/persist(), QuotaExceeded crudo. Chequear navigator.storage.estimate()/persist() + error accionable | `opfs.rs`, `idb.rs` · wasm.md | ❌ Pendiente |
| `MOD-28` | wasm | 🟢 | Nits agrupados: flush() engañoso en backend in-memory, metadata descartada sin señal, CRC fallback débil, next_cursor f64, retry matchea strings, MessagePorts sin close, opfs.rs↔bridge.js duplicados | `lib.rs:1518,1582,943` · wasm.md | ❌ Pendiente |
| `MOD-29` | node | 🔴 | Superficie ~11 ops de ~42 del core: sin grafo, IQL, search_vector, export, métricas. Cerrar por olas (lectura→grafo→mantenimiento) cada una con test roundtrip | `vantadb-node/src/lib.rs` · node.md | ❌ Pendiente |
| `MOD-30` | node | 🔴 | Distribución rota fuera win-x64: un binario .node commiteado al repo, sin optionalDependencies por plataforma. Esquema napi-rs estándar + CI 5 targets | `package.json` · node.md | ❌ Pendiente |
| `MOD-31` | node | 🟢 | Nits agrupados: index.d.ts todo `any`, README referenciado pero inexistente, tests FFI solo 3 happy-path, distance_metric case-divergente, memory_limit=0 silenciado | `index.d.ts`, `lib.rs:25` · node.md | ❌ Pendiente |
| `MOD-32` | memory | 🟡 | Merge/update L1 no atómico: delete-targets ANTES de put-merged → ventana de pérdida en crash. Invertir orden (duplicado transitorio ≫ pérdida de datos) | `core/record/l1_writer.rs:142-148` · memory.md | ❌ Pendiente |
| `MOD-33` | memory | 🟡 | Tres managers de pipeline solapados sin señalizar cuál es canónico. Consolidar o marcar canónico en utils/mod.rs | `utils/{pipeline_manager,stateful_pipeline_manager,pipeline_factory}.rs` · memory.md | ❌ Pendiente |
| `MOD-34` | memory | 🟡 | Handlers scene_read/list/query completos SIN ningún transporte MCP/REST (grep core: 0 refs). Wiring dispatch en vantadb-mcp desbloquea capa knowledge para agentes | `gateway/knowledge_handlers.rs` · memory.md | ❌ Pendiente |
| `MOD-35` | memory | 🟢 | Nits agrupados: single-writer-per-session implícito sin documentar, RFC3339 lexicográfica frágil, LocalStateBackend volátil sin doc, tests/conversation_hook.rs muerto, sanitize 50% re-exports, global_llm_concurrency config muerta (~-300 líneas) | `l0_recorder.rs:145`, `persona_generator.rs:12` · memory.md | ❌ Pendiente |
| `MOD-36` | proxy | 🔴 | `mem:sync`/`mem:create-skill` responden éxito simulado sin efecto alguno (honestidad funcional). Conectar a flush_session (~5 líneas) o corregir texto de respuesta | `mem_command.rs:104-107` · proxy.md | ❌ Pendiente |
| `MOD-37` | proxy | 🟡 | Body limit implícito axum 2MB (413 en prompts largos de Claude Code) + drain SSE sin cap de memoria. DefaultBodyLimit::max(N) explícito + cap 64MB en StreamCapture | `server.rs:288,340-353`, `sse_intercept.rs:60` · proxy.md | ❌ Pendiente |
| `MOD-38` | proxy | 🟡 | Buckets rate-limiter y sesiones crecen monótonamente sin eviction/cap. Eviction LRU o TTL sweep activo | `rate_limit.rs:39`, `session.rs:161` · proxy.md | ❌ Pendiente |
| `MOD-39` | proxy | 🟡 | Tool calls del cliente mezcladas con las nuestras quedan sin responder → 400 upstream. Responder client-tools con "handled client-side" | `memory_tools.rs:146-148` · proxy.md | ❌ Pendiente |
| `MOD-40` | proxy | 🟢 | Nits agrupados: falta test wire de ausencia de x-vanta-user-key upstream, session advance() sin caller, Reporter.add_hook sin hooks, upstream.url sin validar esquema, /v1/responses comparte bucket space_id="", resolve_user_key scan O(10k) hot path | `forward.rs:29-31`, `report.rs:31-50`, `auth.rs:88` · proxy.md | ❌ Pendiente |
| `MOD-41` | providers | 🔴 | Tests de los 3 crates rotos: firma vieja de search + fixture usa `create_namespace` que no existe en vantadb-py (AttributeError seguro en suite ollama). Actualizar a firma actual | `providers/*/tests/test_*.py` · providers.md | ❌ Pendiente |
| `MOD-42` | providers | 🟡 | Sin pyproject/maturin ni wheels: indistribuibles salvo compilación manual (publish=false). Decidir: publicar vía maturin o marcar experimental-interno | `providers/*/Cargo.toml` · providers.md | ❌ Pendiente |
| `MOD-43` | providers | 🟡 | ~85% duplicación entre crates generó API inconsistente (text/payload, cursor naming, limit i32/usize). Crate providers/common unifica contrato por construcción | `openai/src/python.rs` vs `litellm/src/python.rs:131` · providers.md | ❌ Pendiente |
| `MOD-44` | providers | 🟡 | Stubs `.pyi` firman API que ya no existe (search sin namespace, omiten 4 métodos). Regenerar desde firmas reales + anti-drift (patrón MOD-18) | `providers/*/vantadb_*.pyi` · providers.md | ❌ Pendiente |
| `MOD-45` | providers | 🟢 | Nits agrupados: unwrap en litellm embed_fn, READMEs desactualizados ×3, test_init sin importorskip, campo timeout guardado y muerto | `litellm/src/python.rs:131` · providers.md | ❌ Pendiente |
| `MOD-46` | integrations | 🔴 | crewai from_dict pasa string como embedding → TypeError en round-trip to_dict/from_dict; también cursor str→usize TypeError (:164-176) | `integrations/crewai/vectorstore.py:217-218` · integrations.md | ❌ Pendiente |
| `MOD-47` | integrations | 🟡 | langchain add_documents: ids parciales filtrados producen lista corta → ValueError engañoso. Validar longitud antes de filtrar o generar ids faltantes | `integrations/langchain/vectorstore.py:470` · integrations.md | ❌ Pendiente |
| `MOD-48` | integrations | 🟡 | llamaindex: MetadataFilter no importado (revienta con get_type_hints) + attrs privados sin PrivateAttr pydantic. Verificar runtime según versión pydantic | `integrations/llamaindex/vectorstore.py:42-50,362` · integrations.md | ❌ Pendiente |
| `MOD-49` | integrations | 🟡 | openai/ollama gemelos ~95% idénticos (~450 líneas), asimilarity_search fake-async inconsistente. Factorizar módulo común interno | `integrations/{openai,ollama}/vectorstore.py` · integrations.md | ❌ Pendiente |
| `MOD-50` | integrations | 🟢 | Nits agrupados: mem0 score-heurística ambigua, letta sin contrato declarado (experimental?), categorize() DEPRECATED borrable, haystack count_documents O(n) sobre 1M records | `mem0/:45`, `haystack/:370` · integrations.md | ❌ Pendiente |
| `MOD-51` | benches | 🔴 | memory_budget: grupo criterion cronometra un entero (`black_box(inserted)`) — número fabricado. Eliminar brazo o b.iter real sobre get_memory_stats | `benches/memory_budget.rs:147` · benches.md | ❌ Pendiente |
| `MOD-52` | benches | 🟡 | WAL throughput bench ausente: costo de durabilidad never/write/sync inc measurable. Crear wal_throughput: WAL on/off × política fsync | `benches/` (skip_wal=true) · benches.md | ❌ Pendiente |
| `MOD-53` | benches | 🟡 | param_sweep, acorn_filtered_search, bench_concurrent solo stdout: sin gate de regresión. Convertir a criterion groups o exportar JSON artifact nocturno | `benches/*.rs` mains planos · benches.md | ❌ Pendiente |
| `MOD-54` | benches | 🟡 | Cobertura de features vendidas débil: hybrid_queries 96 docs toy, sin bench graph traversal, graphrag bloqueado por stack overflow (AUDIT-04) | `hybrid_queries.rs` · benches.md | ❌ Pendiente |
| `MOD-55` | benches | 🟢 | Nits agrupados: high_density sin seed y escribe DB en CWD del repo, generate_vectors duplicado ×8, recall_at_10 O(n²·dim), stress_test nombre inflado | `high_density.rs`, `incremental_bench.rs:65` · benches.md | ❌ Pendiente |
| `MOD-56` | benchmarks | 🔴 | Ambos baselines de regresión perf VACÍOS: gates nightly/perf comparan contra nada (no-op). Corrida bootstrap workflow_dispatch + commit baselines | `criterion_baseline.json`, `python_baseline.json` · benchmarks.md | ❌ Pendiente |
| `MOD-57` | benchmarks | 🔴 | 128 MB de artefactos DB commiteados a git (blobs persisten en historial). `git rm -r --cached data_comp_bench/` + .gitignore junto a data_bench_db | `benchmarks/data_comp_bench/` (42 archivos) · benchmarks.md | ❌ Pendiente |
| `MOD-58` | benchmarks | 🟡 | Resultados sin trazabilidad: JSON sin fecha/hardware/commit, wasm report sin datos crudos versionados, seeds faltantes en 3 scripts Python, graphrag_bench registrado como example | `vanta_benchmark_report.json`, `WASM_BENCHMARK_REPORT.md` · benchmarks.md | ❌ Pendiente |
| `MOD-59` | cross | 🟡 | Semántica score/distance contradictoria entre transportes (wasm se contradice a sí mismo: lib.rs:953 similitud vs :1024 distancia bajo el mismo campo "score") + límites divergentes 1000× (vector dim 10M vs 10k; top_k 1000 vs 10000). Decisión única en core: exponer ambos campos + constantes centralizadas propagadas a los 5 SDKs | `wasm lib.rs`, `api.rs:1555` · cross-modulos.md | ❌ Pendiente |
| `MOD-60` | cross | 🟡 | Taxonomía de errores de élite del core aplanada a string en TODAS las fronteras FFI (wasm/node/python). Shape serializable {code,message,hint} traducido mecánicamente por cada binding | `wasm lib.rs:1518`, `convert.rs:659` · cross-modulos.md | ❌ Pendiente |
| `MOD-61` | cross | 🟡 | Sin matriz de superficie mínima común: los mismos 5 métodos faltan en TODOS los transportes (remove_edge, versions/supersede, similar_to_key, count, delete_by_filter). Doc de 1 página + test cross-crate que falle si falta método declarado | mcp §5, python H9 · cross-modulos.md | ❌ Pendiente |
| `MOD-62` | cross | 🟡 | Release pipeline solo cubre crates.io: npm (wasm/ts/node) y PyPI (vantadb-py) manuales o inexistentes. Job npm (wasm-pack + optionalDeps) + job PyPI (maturin) en release.yml | `release.yml`, `release-plz.toml:28-32` · cross-modulos.md | ❌ Pendiente |

> **Total P32: 62 tareas** — 14 🔴, 34 🟡, 14 🟢. **Top acciones sistémicas (cross-modulos.md):** (1) durabilidad: MOD-01+02 + persistencia edges WASM (ya CORE-02); (2) puertas de entrada ~30 líneas: MOD-07+12+16+36; (3) contrato único score/límites/errores MOD-59/60 propagado; (4) node MOD-29/30 o declararlo experimental; (5) gates bench reales MOD-56. Secuencia: 🔴 primero (14), luego 🟡 por módulo, nits en batch.
>
> **Nota P33:** la referencia "(P33)" en BND-06 apunta a una campaña informal de otra sesión (MEM-50); P32 es la sección canónica de reviews de módulos.
