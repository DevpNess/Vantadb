...
> **Total open items:** 108 activas (+FIND-59 insert_lock, FIND-58 re-validado — fix 175790a9 no cubre el gate crudo; post-campana quality-gtm-wave 2026-09-03: -11 filas por ejecucion RES-07/03/12/15C/09 + GOV-TK1/9 + MKT-18h/f/i + SRV-07 + AUD-045, +3 FIND-56/57/58 nuevos, GOV-TK1/MKT-18f re-escaladas; split negocio: 15 filas a Backlog-negocio.md; previo 126)
---
...
| FIND-53 | Baja | Doc drift: tool MCP `embed_texts` (EMB-05, commit `241b0868`) sin sección en `docs/api/MCP.md` — `scripts/validate-docs-coverage.ps1` reportaba 1/49 gaps (2026-09-03, hallazgo colateral GOV-TK9) → ✅ RESUELTA 2026-09-03: fila añadida en familia Search & Query (8) + header 49/49, commit `f4379770`, coverage 0 gaps | `docs/api/MCP.md` | 🟢 | 🟢 Baja | ✅ Completada |
...
| FIND-57 | Media | **`AsyncIngestionPipeline::new` default `worker_count=4` es activo-perjudicial (origen: medición RES-03 2026-09-03):** bench nuevo `benches/ingestion_concurrent.rs` muestra −31% (w=2) y −43% (w=4) de ops/s vs w=1 en régimen de inserción serial del motor (insert_lock global + WAL fsync por escritura); el cuello no es el canal. Fix requiere re-medir con payloads realistas (dim alto/text) y cambiar default es decisión de behavior/API → Gate D. Ver `docs/operations/BENCHMARKS.md` §13 | `src/ingestion.rs:40`, `benches/ingestion_concurrent.rs` | 🟡 ~2h | 🟡 Media | ⬜ Pendiente |
| FIND-58 | Baja | **Gate wasm32 crudo roto por resolución de `getrandom` (origen: RES-03 2026-09-03):** `cargo check -p vantadb --target wasm32-unknown-unknown --no-default-features --features wasm` falla en el dep `getrandom@0.3.4` (`wasm_js` feature no unificada; `--cfg getrandom_backend="wasm_js"` solo no basta); el check aislado del crate pasa → es feature-resolution del árbol vantadb. FIND-52 registró "cargo check wasm32 0" sin comando exacto → documentar gate reproducible (comando + RUSTFLAGS/feature) o fix de unificación. Colateral misma clase: `src/sdk/search/debug_ops.rs` emite 4 warnings unused-import solo en perfil release (imports bajo `cfg(debug_assertions)`) | `Cargo.lock`, `vantadb-wasm/Cargo.toml`, `src/sdk/search/debug_ops.rs` | 🟡 1-2h | 🟢 Baja | ⚠️ 2026-09-03 re-verificado: el fix paralelo 175790a9 (wasm_js cfg en .cargo/config.toml) desbloquea wasm-pack pero ESTE gate sigue rojo: ``cargo check -p vantadb --target wasm32-unknown-unknown --no-default-features --features wasm`` → getrandom lib error. Falta cubrir la combinación de features del gate crudo. |
| FIND-59 | Media | **Serialización global ``insert_lock`` del motor en la ruta de escritura (origen: medición RES-03 2026-09-03, BENCHMARKS §13):** con el canal multi-consumidor ya óptimo, el techo de ingesta (~113.5 ops/s, y PEOR con más workers) lo pone el lock global de inserción + fsync WAL. La mitad WAL es FUT-12; esta mitad no está trackeada. Acción: DISCOVERY vanta-arch (analizar granularidad: sharding por namespace/region lock vs RCU-write-path; riesgo: concurrencia R1/R2 + durabilidad) → si el análisis muestra ganancia >2× sobre el baseline §13, task de implementación separado. No tocar sin bench A/B contra §13 (Regla 9) | ``src/storage/engine/insert.rs``, ``src/storage/engine/mod.rs``, BENCHMARKS §13 | 🟠 análisis 1-2d | 🟡 Media | ⬜ Pendiente |
