...
- **Estado:** ⏳ PLAN CREADO — Pendiente de aprobación del usuario (no ejecutar hasta aprobación explícita).
- **Próximo:** tras aprobación, Wave 0 (WEB-00) + Wave 1 (WEB-01/02/03) en paralelo — archivos disjuntos (desktop/src vs src/cli_server.rs) — con sub-agentes vanta-worker, verify mecánico del lead entre tareas, commits selectivos.