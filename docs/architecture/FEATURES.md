...
| `server` | 🟡 opt-in · ⚠️ experimental | Server HTTP embebido: `circuit_breaker`, `cli_server`, `connection_pool` (requiere `cli`) | `src/lib.rs:66,72,78` (6 gates). No en features de test de CI; `vantadb-server` la activa (`vantadb-server/Cargo.toml:10`) |
