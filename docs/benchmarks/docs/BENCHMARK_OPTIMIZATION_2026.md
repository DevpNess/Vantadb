...
12. **search_layer regresa +66~90% a ef_search ≥200 — causa raíz: clonación de neighbor lists en el fallback** — el 2026-07-31 con CPU 49% (limpio), hnsw_recall_ef confirmó: search ef_10-100 flat (310-318ms), pero **ef_200 = 589ms (+90% vs baseline 294-310ms) y ef_400 = 515ms (+66%)**. La regresión ES REAL: empeoró con entorno más limpio que el run de ayer (412/391ms), descartando ruido. **Mecanismo:** en `search_layer` (src/index/search.rs:220-233), cuando el inline cache `n.neighbor_lists.get(layer)` está vacío, cae al fallback `self.neighbor_index.get_neighbors(cand_id, layer)` (src/index/neighbor_index.rs:42) que hace `map(|v| v.clone())` — **clona la lista completa de vecinos en CADA candidato**. A mayor ef, más candidatos → más clones → regresión proporcional a ef. **Confirmado por el patrón exacto de los datos.** `take_nl()`/`give_nl()` (E2) mitiga la allocación pero el inline cache y el fallback siguen copiando. **Candidato a fix:** devolver referencia/iterador en vez de clonar, o popular el inline cache en el path de insert.
