...
- **Deuda abierta:** (1) verificación end-to-end del build en CI al primer dispatch (pre-mortem del plan); (2) `vantadb-server/Dockerfile` alternativo está roto igual (`COPY vantadb/Cargo.toml` — crate inexistente; asset `vantadb-server-…-amd64` que el release no sube con ese nombre) → `hardening.md` §5 lo documenta como si funcionara; candidato FIND si el orquestador lo decide; (3) push a registry cuando haya decisión de marca + credenciales.
