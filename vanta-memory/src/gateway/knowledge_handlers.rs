...
//!
//! LLM-free (Principio 4): `scene_query` ranks by keyword-term overlap
//! (`significant_terms`), same documented ceiling as `recall_candidates`.

use serde::{Deserialize, Serialize};
use thiserror::Error;

use crate::core::record::l1_reader::overlap_score;
...
///
/// # ponytail: naive overlap query, no vector search
/// Same degradation ceiling as `recall_candidates` (l1_reader): cheap
/// token-overlap gate, no embeddings. Upgrade path: wire the VantaDB vector
/// index once the core search API is exposed to this crate.
pub fn scene_query(
    db: &vantadb::sdk::VantaEmbedded,
    ...
    let mut hits: Vec<SceneQueryHit> = read_blocks(db, &request.session_key)?
        .into_iter()
        .filter(|block| !block.is_deleted())
        .map(|block| {
            let haystack = format!("{} {}", block.meta.summary, block.content);
            (block, overlap_score(&haystack, keyword))
        })
        .filter(|(_, score)| *score > 0)
        .map(|(block, score)| SceneQueryHit {
    ...
    hits.sort_by(|a, b| b.score.cmp(&a.score).then_with(|| b.heat.cmp(&a.heat)));
    hits.truncate(request.top_k.unwrap_or(DEFAULT_QUERY_TOP_K));
    Ok(SceneQueryResponse { hits })
}
