        ...
        #[cfg(feature = "embed-local")]
        {
            if self.query_vector.is_none() {
                ...
                if let Ok(provider) = crate::llm::LocalOnnxProvider::new(&model_dir) {
                    ...
                    {
                        self.query_vector =
                            Some(crate::node::VectorRepresentations::Full(vec));
                    }
                }
            }
        }
...
mod tests {
    ...
    fn test_physical_vector_refine_passthrough_no_embedding() {
        // Without `remote-inference`/`embed-local`, query_vector stays None → passthrough.
        // With `embed-local`, query_vector = Some(deterministic) → nodes without vectors filtered out.
        ...
        if cfg!(feature = "embed-local") {
            // Deterministic embed always succeeds → vectors present → nodes without vectors filtered
            assert!(
                refine.next().unwrap().is_none(),
                "with embed-local, nodes without vectors must be filtered out"
            );
        } else if cfg!(feature = "remote-inference") {
            // Ollama likely unavailable in CI → passthrough or filtered, accept either without asserting
            let _ = refine.next().unwrap();
        } else {
    ...
    fn test_physical_vector_refine_open_close_cycle() {
        // With embed-local, need a vector node that matches the query to pass the cosine filter.
        #[cfg(feature = "embed-local")]
        let nodes = {
            let provider =
                crate::llm::LocalOnnxProvider::new("embeddings/models/multilingual-e5-small/onnx")
                    .unwrap_or_else(|_| crate::llm::LocalOnnxProvider::new_dummy(384));
            let q_vec = crate::llm::EmbeddingProvider::embed(&provider, "query").unwrap();
            let mut n = UnifiedNode::new(1);
            n.vector = crate::node::VectorRepresentations::Full(q_vec);
            vec![n]
        };
        #[cfg(not(feature = "embed-local"))]
        let nodes = vec![bool_node(1, true)];

        let child = MockScan::new(nodes.clone());
        let mut refine = PhysicalVectorRefine::new(Box::new(child), "query".into(), 0.5);
        refine.open().unwrap();
        assert!(refine.next().unwrap().is_some(), "should return node (passthrough or matched)");
        refine.close().unwrap();
        // Re-open — MockScan resets to saved vec, so second cycle must also return something
        let child2 = MockScan::new(nodes);
        let mut refine2 = PhysicalVectorRefine::new(Box::new(child2), "query".into(), 0.5);
        refine2.open().unwrap();
        assert!(refine2.next().unwrap().is_some());
        refine2.close().unwrap();
    }
