...
├─ Completo → .antigravity\task-system\harness\harness-executor.ps1 -PlanFile ...
├─ Una tarea → .antigravity\task-system\harness\harness-executor.ps1 -PlanFile ... -SingleTask DRV-NN
└─ En paralelo → .antigravity\task-system\harness\harness-executor.ps1 -PlanFile ... -Parallel
```
...
.antigravity/task-system/prompts/plan.md               ← crear plan desde backlog (triage gate)
.antigravity/task-system/prompts/task.md               ← definir tarea a profundidad
.antigravity/task-system/prompts/iter-loop-tools.md               ← ejecutar una iteración del harness
.antigravity/commands/pipeline.md                      ← entry point: plan | task | run | interactive
.antigravity/task-system/harness/harness-executor.ps1   ← loop externo PowerShell
SKILL.md                      ← referencia completa del skill
tasks/<ID>.md               ← auto-generated task definitions (resuelve a .antigravity/skills/campaign-executor/tasks/<ID>.md)
.antigravity/references/           ← repos clonados (awesome-harness-engineering, statewright, ...)
```
...
Antigravity opera por turnos (Request-Response). Cada invocación ejecuta
...
| **agent** | Antigravity con los MCP tools de campaign |
...
```yaml
- name: Lurkr scanner
  run: npx lurkr scan .antigravity/ --report-format json
```
