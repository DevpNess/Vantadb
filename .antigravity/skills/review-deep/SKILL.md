...
El archivo temporal `.antigravity/skills/review-deep/tmp/${MODULE}-findings.json`
guarda hallazgos intermedios para no perder contexto entre iteraciones.

### Scorecard por Iteración (darwin-godel pattern)

Cada iteración del loop registra una entrada en `.antigravity/skills/review-deep/tmp/${MODULE}-<timestamp>.json`:
...
```
/loop-goal --prompt-file .antigravity/skills/review-deep/loop-prompt.md "MODULE=..." "DEPTH=full"
  │
