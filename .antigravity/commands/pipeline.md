...
> Path resolution: `prompts/X.md` → `.antigravity/task-system/prompts/X.md`
> Skills: `skills/X` → `.antigravity/skills/X/`
> Tasks: `tasks/ID.md` → `.antigravity/skills/campaign-executor/tasks/ID.md`
...
2. Si no existe task file → cargá `prompts/task.md` y ejecutá sus 4 fases:
   ...
   - Creá `.antigravity/skills/campaign-executor/tasks/<ID>.md`
...
**Al finalizar, mostrá:**
```
Task file creado: .antigravity/skills/campaign-executor/tasks/<ID>.md
...
```
/loop-goal "Ejecutá UNA TAREA COMPLETA siguiendo `.antigravity/task-system/prompts/pipeline-full.md`"
```
...
```
/loop-goal "Ejecutá UNA iteración siguiendo `.antigravity/task-system/prompts/iter-loop-tools.md`"
```
...
```
.antigravity\task-system\harness\harness-executor.ps1 -PlanFile docs\plans\<plan-más-reciente>.md -Interval 10
```
...
| Después de... | Mostrar... |
|--------------|------------|
| Crear plan | `/pipeline run` *(backlog completo)* o `/build` *(primera tarea, RED→GREEN)* o `.antigravity\task-system\harness\harness-executor.ps1` *(terminal)* |
...
| 1 | **Una iteración** (paso a paso con MCP) | `/loop-goal "Ejecutá UNA iteración siguiendo \`.antigravity/task-system/prompts/iter-loop-tools.md\`"` |
