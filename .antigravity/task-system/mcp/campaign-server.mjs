...
server.tool(
  ...
  async ({ planFile, interval, stallThreshold, singleTask, timeout, parallel, maxParallel, model, dryRun }) => {
    const harnessPath = join(PROJECT_ROOT, ".antigravity", "task-system", "harness", "harness-executor.ps1")
    if (!existsSync(harnessPath)) {
      return { content: [{ type: "text", text: JSON.stringify({ error: "harness-executor.ps1 not found at .antigravity/task-system/harness/" }) }] }
    }
