...
📖 **`.antigravity/VANTADB-OPERATING-MANUAL.md`** — Manual de Operación completo (948 líneas, 14 secciones)
...
El sistema de pipeline vive en `.antigravity/` y se activa cuando el usuario envía un comando.

### Entry Points

| Mensaje del usuario | Archivo a leer | Modos |
|---|---|---|
| `/pipeline ...` | `.antigravity/commands/pipeline.md` | Plan / Task / Run / Interactive / Pipeline / Ejecución |
| `/audit ...` | `.antigravity/commands/audit.md` | Full / Quick / Certify / Review |
| `/build ...` | `.antigravity/commands/build.md` | Default / Auto / Prove |
| `/ship` | `.antigravity/commands/ship.md` | Fan-out GO/NO-GO |
| `/rollback` | `.antigravity/commands/rollback.md` | Revertir ship fallido |
| `/status` | `.antigravity/commands/status.md` | Dashboard de un vistazo |
| `/spec` | `.antigravity/commands/spec.md` | Spec-Driven Development |
| `/webperf` | `.antigravity/commands/webperf.md` | Web performance audit |
| `/code-simplify` | `.antigravity/commands/code-simplify.md` | Simplify code |
...
2. **Leer entry point:** el agente LEE el archivo de comando correspondiente en `.antigravity/commands/` con Read tool
...
| `campaign-executor` | En modo task / run (no es skill instalado, leer `.antigravity/skills/campaign-executor/SKILL.md`) |
...
| Skill | Comando Antigravity | Qué hace |
|-------|-----------------|----------|
...
Skills de ingeniería instaladas desde [addyosmani/agent-skills](https://github.com/addyosmani/agent-skills) (`.antigravity/`). Son **workflows obligatorios** — el agente DEBE usarlos cuando apliquen. No saltarse pasos.
...
**Personas especializadas** (`.antigravity/agents/`): `vanta-audit` (Security + Code Review), `vanta-chaos` (Fuzzing + Resilience), `vanta-tuner` (Performance + Observability). Legacy `code-reviewer`, `security-auditor`, `test-engineer` eliminados — reemplazados por vanta-*.

**Referencias** (`.antigravity/references/`, **12 total**): `definition-of-done.md`, `testing-patterns.md`, `security-checklist.md`, `performance-checklist.md`, `accessibility-checklist.md`, `observability-checklist.md`, `orchestration-patterns.md`, `REFERENCE-SYNTHESIS.md`, `awesome-harness-engineering/`, `darwin-godel-machine/`, `deepclaude/`, `statewright/`.
...
2. **Mapear todas sus referencias** — grep por el nombre del archivo en TODO el workspace (no solo `.antigravity/`)
...
- **Personas** (`.antigravity/agents/<role>.md`) — roles con perspectiva y formato de output. El *who*.
- **Comandos** (`.antigravity/commands/*.md`) — entry points del usuario. El *when*. Capa de orquestación.
...
| **Campaign** | `node .antigravity/task-system/mcp/campaign-server.mjs` | Task system: 30+ tools para plan, task, verify, state machine |
